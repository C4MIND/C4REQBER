...
10. **Flash `--sources`:** `blast flash` / MCP `blast_flash` share `run_flash`. A source is **verified** only after `CitationVerifier` (CrossRef DOI and/or OpenAlex title similarity ≥ 0.82 → `VERIFIED`/`PARTIAL`). Presence of a DOI/URL alone is **checkable**, not verified. Footer / mascot / `sources` list = **verified only**. Unverified raw hits may appear under a separate “Unverified hits (not counted)” section. If verified sources exist, the answer must not claim “unable to find”. Strip `example.com` and `scholar.google.com/scholar?q=` URLs. `~/.c4reqber/secrets.env` is loaded for **all** CLI commands so Tavily can activate. **Search meta contract:** `MultiSourceSearcher.search_all` returns `sources_used` as an **int count** and `source_names` as a **list[str]** — flash meta / CLI must normalize via `source_names_from_result` (never `list(int)`). Tavily meta is `on|off|no_key`. Missing `sentence-transformers` → lexical dedup fallback (`dedup=lexical_fallback`), quiet info — not a gather crash.
...
13. **Windows console:** CLI startup reconfigures stdout/stderr to UTF-8 (`errors=replace`). Cube mascot falls back to ASCII glyphs when the active code page cannot encode `◈▣✓` — never raise `UnicodeEncodeError: 'charmap'`.
