    ...
    while i < len(lines):
        ...
        if stripped.startswith('except '):
            ...
            for j in range(i - 1, -1, -1):
                ...
                if prev_indent < indent and (prev_stripped.startswith('try:') or prev_stripped.startswith('def ') or prev_stripped.startswith('async def ') or prev_stripped.startswith('class ')):
                    break
                
                if prev_indent == indent and prev_stripped.startswith('except '):
                    prev_match = re.match(r'\s*except\s+\(?([^:]+)\)?\s*(?:as\s+\w+)?\s*:', prev_line)
                    if prev_match:
