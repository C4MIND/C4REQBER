...
class JWTAuthMiddleware(BaseHTTPMiddleware):
    ...
    Uses AuthManager.decode_token so issuer + revocation match the dependency path
    (bare jwt.decode previously skipped both).
    """
    ...
    async def dispatch(self, request: Request, call_next) -> JSONResponse:
        ...
        body = b""
        if request.url.path.startswith("/v8/webhook") or request.url.path.startswith(
            "/api/v8/webhook"
        ):
        ...
        try:
            secret = os.getenv("JWT_SECRET") or os.getenv("C4REQBER_JWT_SECRET")
            if not secret:
            ...
            from src.api.auth import AuthManager, MissingJWTSecretError
