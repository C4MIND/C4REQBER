...
def _normalize_discovery_for_markdown(discovery: dict[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(c4_path, dict):
        ...
        c4_path_list = []
    contradiction = discovery.get("contradiction") or str(discovery.get("triz_result", {}).get("contradiction", "")) or str(discovery.get("contradiction_mining", {}).get("contradiction", "")) or "N/A"
    confidence = discovery.get("confidence") or discovery.get("best_hypothesis", {}).get("confidence", 0.0)
    if not isinstance(confidence, (int, float)):
        confidence = float(confidence)
    falsifiability = discovery.get("falsifiability_criteria") or discovery.get("falsifier", {}).get("counter_args", [])
    if not isinstance(falsifiability, list):
        falsifiability = []
    return {"created_at": discovery.get("created_at") or discovery.get("timestamp") or datetime.now().isoformat(), "domain": discovery.get("domain", "general"), "confidence": confidence, "problem": discovery.get("problem", "N/A"), "contradiction": contradiction, "hypothesis": hypothesis_text, "c4_path": c4_path_list, "falsifiability_criteria": falsifiability}
...
def export_discovery(req: ExportRequest) -> ExportResponse:
    ...
    if fmt not in supported:
        raise HTTPException(status_code=400, detail=f"Unsupported format: {fmt}. Use: {', '.join(sorted(supported))}")
    ...
    try:
        if fmt == "markdown":
            ...
            if not bibtex:
                raise HTTPException(status_code=400, detail="No bibtex field found in discovery. Include paper.bibtex or bibtex data.")
            ...
            if paper.get("error"):
                raise HTTPException(status_code=400, detail="Paper generation failed in the discovery. Cannot export LaTeX.")
            hypothesis = req.discovery.get("hypothesis", {})
            if not hypothesis:
                raise HTTPException(status_code=400, detail="No hypothesis found in discovery. Cannot export LaTeX.")
            from .pipeline import generate_paper
            paper_parts = generate_paper(hypothesis=hypothesis, papers=req.discovery.get("papers", []), proof=req.discovery.get("proof", {}))
            latex_content = paper_parts.get("latex", "")
            if not latex_content:
                raise HTTPException(status_code=400, detail="LaTeX generation returned empty content.")
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            fp = EXPORT_DIR / f"discovery_{timestamp}.tex"
            fp.write_text(latex_content)
            filepath = str(fp)
        elif fmt == "html":
            confidence = req.discovery.get("confidence") or req.discovery.get("best_hypothesis", {}).get("confidence", 0.0)
            if not isinstance(confidence, (int, float)):
                confidence = float(confidence)
            stats = {"discoveries": 1, "patterns": len(req.discovery.get("triz_principles", [])), "validated": 1 if req.discovery.get("status") == "complete" else 0, "avg_confidence": confidence}
