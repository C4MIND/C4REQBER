...
"""Discovery pipeline functions — extracted from discovery_v8.py for God Object decomposition."""
...
def _build_dissertation(discovery: dict, attempts: list) -> dict:
    ...
    return {**discovery, "dissertation": {"title": f"On the {discovery.get('problem', 'Unknown Problem')[:100]}", "abstract": hyp_text[:500] if hyp_text else "Abstract pending", "keywords": discovery.get("keywords", []), "sections": [{"heading": "1. Introduction", "content": f"Background: {hyp_text[:300]}"}, {"heading": "2. Literature Review", "content": f"Gaps identified: {discovery.get('gap_miner', {}).get('gaps_found', 0)} research gaps"}, {"heading": "3. Hypothesis", "content": hyp_text[:1000] if hyp_text else "See hypothesis section"}, {"heading": "4. Methodology", "content": f"C4-CDI-Turbo pipeline: {discovery.get('thresholds_applied', {})}"}, {"heading": "5. Predictions", "content": "Falsifiable predictions from hypothesis generation"}, {"heading": "6. Validation", "content": f"Self-critique: {discovery.get('self_critique', {}).get('recommendation', 'N/A')}"}, {"heading": "7. Implications", "content": "Implications for theory and practice"}, {"heading": "8. Conclusion", "content": "Summary of findings and future work"}], "references": discovery.get("papers", [])[:20], "attempts": attempts, "citation": "Selyutin I., Kovalev N.I. (2026). Generated by C4-CDI-Turbo v8.2 Dissertation Mode."}}
...
async def flash_discovery(
    ...
    *,
    ...
    domain = request.domain
    errors: list[str] = []
    results: dict[str, Any] = {"problem": problem, "domain": domain, "pipeline_version": "v8.0-flash", "warnings": []}
...
async def multi_hypothesis_discovery(request: MultiHypothesisRequest) -> dict[str, Any]:
    ...
    for i in range(count):
        try:
            h = await generate_hypothesis(problem=problem, c4_path=c4_path, triz_principles=triz_principles, papers=papers)
            h["index"] = i
            h["variant"] = f"variant_{i}"
            hypotheses.append(h)
        except Exception as e:
            errors.append(f"hypothesis[{i}]: {str(e)}")
            hypotheses.append({"index": i, "variant": f"variant_{i}", "source": "error", "text": f"Failed: {str(e)}", "error": str(e)})

    # P2.5: Rank hypotheses before simulation using MCDM
    valid_hypotheses = [h for h in hypotheses if h.get("text") and "Failed" not in h.get("text", "")]
    ...
    try:
        ...
        for i, rh in enumerate(ranked_objs):
...
async def dissertation_mode(request: DissertationRequest) -> dict[str, Any]:
    ...
    for iteration in range(1, request.max_iterations + 1):
        ...
        try:
            discovery_result = await one_click_discovery(OneClickRequest(problem=current_problem, domain=request.domain))
        except Exception as e:
        ...
        novelty = discovery_result.get("novelty", {}).get("novelty_score", discovery_result.get("novelty_check", {}).get("novelty_score", 0))
        hypothesis = discovery_result.get("hypothesis", {})
        hyp_text = hypothesis.get("text", "")[:300]
        quality = gap_potential * 0.3 + novelty * 0.3 + (0.7 if critique_rec in ("PUBLISH", "NEEDS_MORE_EVIDENCE") else 0.2) * 0.4
        attempt = {"iteration": iteration, "status": status, "gap_potential": gap_potential, "novelty": round(novelty, 3), "critique_recommendation": critique_rec, "quality_score": round(quality, 3), "abort_reasons": abort_reasons, "hypothesis": hyp_text, "time_seconds": round(time.perf_counter() - t_iter, 1)}
        attempts.append(attempt)
        if quality > best_score:
            best_score = quality
            best_result = discovery_result
        gates_passed = (status != "aborted" and gap_potential >= 0.3 and (critique_rec in ("PUBLISH", "NEEDS_MORE_EVIDENCE")))
        if gates_passed:
        ...
        if iteration < request.max_iterations:
            try:
                ...
                if refined and len(refined) > 20:
                    ...
                    strategies = [lambda p: f"What is the mechanistic basis of {p}?", lambda p: f"How does {p} vary across scales (molecular to ecological)?", lambda p: f"What is the evolutionary origin of {p}?", lambda p: f"How can we computationally model {p} to predict outcomes?", lambda p: f"What overlooked variable explains contradictions in {p}?"]
                    current_problem = strategies[(iteration - 1) % len(strategies)](request.problem)
            except (ImportError, ModuleNotFoundError, RuntimeError, OSError):
                current_problem = f"New perspective on {request.problem[:100]}: iteration {iteration}"
    return {"status": "dissertation_failed", "message": f"Unable to find publishable hypothesis after {request.max_iterations} iterations. Best quality: {best_score:.2f}", "attempts": attempts, "best_result": best_result, "total_time_seconds": round(time.perf_counter() - start_total, 2)}
