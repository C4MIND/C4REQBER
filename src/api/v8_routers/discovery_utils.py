...
def _domain_improving_param(domain: str) -> str:
    return {"physics": "speed", "chemistry": "concentration", "biology": "adaptability",
            "engineering": "reliability", "materials": "strength", "electronics": "power",
            "energy": "efficiency", "medicine": "precision", "economics": "productivity",
            "software": "speed"}.get(domain, "efficiency")


def _domain_worsening_param(domain: str) -> str:
    return {"physics": "stability", "chemistry": "cost", "biology": "complexity",
            "engineering": "weight", "materials": "cost", "electronics": "heat",
            "energy": "cost", "medicine": "side_effects", "economics": "risk",
            "software": "complexity"}.get(domain, "cost")
...
def run_fra_routing(problem: str) -> dict[str, Any]:
    ...
    try:
        ...
        return {"situation": fp["situation"], "c4_state": str(fp["c4_state"]),
                "recommended_operators": fp["recommended_operators"],
                "situation_scores": fp["scores"], "routed_chain": router.route(fp["situation"], gap_pct=50.0)}
...
def run_c4_observer(problem: str, c4_path: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        return {"observer_positions": {pos.name: {
...
async def search_isomorphisms(problem: str, papers: list, triz: list) -> dict[str, Any]:
    ...
    try:
        ...
        for iso in ISOMORPHISM_SEED:
            if any(t in problem_lower for t in iso.get("target", "").split("_")) or \
               any(t in problem_lower for t in iso.get("source", "").split("_")) or \
               any(t in problem_lower for t in iso.get("applications", "").split(",")):
...
def mine_contradictions(papers: list) -> dict[str, Any]:
    ...
    try:
        ...
        claims = []
        for p in papers:
            claims.extend(extractor.extract(f"{p.get('title', '')}. {p.get('abstract', '')}",
                                         source=p.get("source", "unknown")))
        contradictions = detector.find_all_contradictions(claims) if len(claims) >= 2 else []
        return {"claims_extracted": len(claims), "contradictions_found": len(contradictions),
                "top_contradictions": [{"claim_a": c.claim_a.text[:120], "claim_b": c.claim_b.text[:120],
    ...
    try:
        ...
        for i, p in enumerate(papers[:10]):
            try:
                ...
                year = 2025
            kg.add_claim(TimeStampedClaim(id=f"claim_{i}", text=p.get("title", ""), timestamp=datetime(year, 1, 1),
                                      source=p.get("source", "unknown"), domain=p.get("domain", "")))
        traj = kg.consensus_trajectory(problem[:80])
        return {"claims_indexed": len(kg.claims), "stability": traj.get("stability", 0.0),
...
def detect_paradigm_shift(papers: list, domain: str) -> dict[str, Any]:
    ...
    try:
        ...
        claims = [ScientificClaim(text=p.get("title", ""), timestamp=datetime(2025, 1, 1),
                              source=p.get("source", "unknown"), domain=domain) for p in papers[:15]]
        warning = detector.analyze(claims, domain=domain)
        return {"paradigm_shift_probability": round(warning.probability, 4),
                "timeframe": warning.estimated_timeframe, "crisis_severity": warning.confidence,
...
def run_strong_inference(problem: str, domain: str, hypothesis: dict) -> dict[str, Any]:
    ...
    try:
        ...
        result = engine.run(problem=problem, hypotheses=generate_competing_hypotheses(problem, domain, count=3), domain=domain)
        return {"method": "Platt's Strong Inference (1964)", "cycles": result.cycles,
...
def run_abduction(problem: str, domain: str, papers: list) -> dict[str, Any]:
    ...
    try:
        ...
        obs = [Observation(description=p.get("title", "")[:100], source=p.get("source", "unknown"), confidence=0.9)
                for p in papers[:5]] or [Observation(description=problem[:100], source="problem")]
        result = engine.infer_to_best_explanation(observations=obs, domain=domain, max_hypotheses=4)
        return {"method": "Inference to the Best Explanation (Peirce)",
                "hypotheses_generated": len(result.hypotheses),
                "best_score": round(result.best_explanation.overall_score, 4) if result.best_explanation else 0.0}
    except (ImportError, AttributeError, TypeError) as e:
        return {"error": str(e)}
...
def run_bayesian_model_averaging(hypothesis: dict, monte_carlo: dict) -> dict[str, Any]:
    ...
    try:
        ...
        return {"weighted_prediction": round(result.weighted_prediction, 4),
...
def run_dempster_shafer(hypothesis: dict, papers: list) -> dict[str, Any]:
    """Step 6.5g: Dempster-Shafer."""
    try:
        from bayesian.dempster_shafer import EvidenceSensor, FrameOfDiscernment
        frame = FrameOfDiscernment(["supported", "refuted", "untested"])
        h_text = hypothesis.get("text", "")
        support = sum(1 for kw in ["improve", "novel", "outperform"] if kw in h_text.lower()) + 1
        bba = EvidenceSensor(frame).from_likelihoods(
            {"supported": support, "refuted": 1.0, "untested": 1.0}, uncertainty=0.2)
        return {                "belief_supported": round(bba.belief({"supported"}), 4)}
    except (ImportError, AttributeError, TypeError) as e:
        return {"error": str(e)}


def run_bayesian_conjugate_update(monte_carlo: dict) -> dict[str, Any]:
    """Step 6.5h: Bayesian Conjugate."""
    try:
        import numpy as np

        from bayesian.core import normal_normal
        result = normal_normal(data=np.array(np.random.default_rng().normal(0.65, 0.12, 20), dtype=np.float64),
                              mu_prior=0.5, tau_prior=10.0, sigma_known=0.15, credible_level=0.95)
        return {"posterior_mean": round(result.mu_post, 4)}
    except (ImportError, AttributeError, TypeError) as e:
        return {"error": str(e)}
...
def run_doe_design(domain: str) -> dict[str, Any]:
    ...
    try:
        ...
        config = DoEConfig(factors=[Factor(name="a", low=0.0, high=100.0, levels=2),
                                   Factor(name="b", low=0.0, high=100.0, levels=2)],
                            design_type=DesignType.LATIN_HYPERCUBE, samples=8)
...
def run_reproducibility_check(problem: str) -> dict[str, Any]:
    ...
    try:
        ...
        check_results = validator.quick_validate(data_available=True, code_available=True, env_specified=True)
...
def run_consensus_meter(hypothesis: dict, papers: list) -> dict[str, Any]:
    ...
    try:
        ...
        h_text = hypothesis.get("text", "")
        evidence_list = [Evidence(source=p.get("source", "unknown"),
                               type=EvidenceType.SUPPORTING if i % 2 == 0 else EvidenceType.NEUTRAL,
                               strength=EvidenceStrength.MODERATE, description=p.get("title", "")[:100])
...
def run_matrix_dream(problem: str, c4_path: dict) -> dict[str, Any]:
    ...
    try:
        ...
        return {"patterns_applied": len(matches),
                "top_patterns": [{"id": p.id, "name": p.name, "score": round(s, 3)} for p, s in matches[:5]]}
    except (ImportError, AttributeError) as e:
        return {"error": str(e)}


async def run_autoscanner(papers: list) -> dict[str, Any]:
    """Step 5.5x: AutoScanner."""
    try:
        from src.discovery.autoscanner import AutoScanner
        candidates = await AutoScanner().scan_local()
...
async def run_cognitive_plugins(problem: str, hypothesis_text: str, domain: str) -> dict[str, Any]:
    ...
    plugins = [("swot", "SWOT"), ("red_team", "Red Team"), ("six_hats", "Six Hats"),
               ...
               ("ishikawa", "Ishikawa"), ("morphological", "Morphological"),
               ("delphi", "Delphi"), ("ooda", "OODA"), ("pre_mortem", "Pre-Mortem"),
               ("second_order", "Second-Order"), ("lateral_thinking", "Lateral"),
               ("design_thinking", "Design"), ("first_principles", "First"),
               ("inversion", "Inversion"), ("constraint_relaxation", "Constraint"),
               ("analogical_reasoning", "Analogical"), ("bayesian_update", "Bayesian"),
    ...
    for plugin_name, display_name in plugins:
        try:
            ...
            if hasattr(module, "execute"):
                results[plugin_name] = {"name": display_name, "status": "success",
                                         "result": module.execute(context=f"{problem} {hypothesis_text}", problem=problem[:200], domain=domain)}
            else:
    ...
    return {"plugins_run": len(plugins), "successful": success, "results": results}
...
def _normalize_discovery_for_markdown(discovery: dict) -> dict:
    ...
    hypothesis_text = hypothesis.get("text", str(hypothesis)) if isinstance(hypothesis, dict) else str(hypothesis)
    c4_path = discovery.get("c4_path", {})
    c4_list = c4_path.get("operators", [c4_path.get("summary", "")]) if isinstance(c4_path, dict) else (c4_path if isinstance(c4_path, list) else [])
    return {"created_at": discovery.get("created_at") or datetime.now().isoformat(),
            "domain": discovery.get("domain", "general"), "problem": discovery.get("problem", "N/A"),
            "hypothesis": hypothesis_text, "c4_path": c4_list}


def _build_dissertation(discovery: dict, attempts: list) -> dict:
    """Build dissertation from src.discovery."""
    hypothesis = discovery.get("hypothesis", {})
    hyp_text = hypothesis.get("text", "")
    return {**discovery, "dissertation": {
        "title": f"On the {discovery.get('problem', 'Unknown')[:100]}",
        "abstract": hyp_text[:500] if hyp_text else "Pending",
        "sections": [{"heading": f"{i}. {s}", "content": f"{s} content"}
                      for i, s in enumerate(["Introduction", "Literature", "Hypothesis", "Methodology", "Results", "Conclusion"], 1)],
        "attempts": attempts,
        "citation": "Generated by C44TCDI."
    }}
