...
def write_config(values: dict[str, str]) -> Path:
    from src.config.paths import ensure_config_dir
...
def run_init_wizard(*, force: bool = False) -> Path:
    ...
    language = Prompt.ask(
        "UI language (en/ru/zh/de/ja/hi/ar)", default=existing.get("language", "en")
    )
    ...
    console.print(
        "[dim]Tip: blast config user --show   |   blast config --show   |   re-run 'blast init'[/]"
    )
    return path
