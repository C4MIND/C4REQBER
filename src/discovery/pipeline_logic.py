    ...
    try:
        ...
        if not results:
            results = [
                ...
                {"id": 2, "name": "Extraction", "description": "Extract the disturbing part"},
                {
                    "id": 3,
                    "name": "Local Quality",
                    "description": "Change from uniform to non-uniform",
                },
            ]
        return results
    except ModuleNotFoundError:
        return [
            {"id": 1, "name": "Segmentation", "description": "Fallback"},
            {"id": 2, "name": "Extraction", "description": "Fallback"},
            {"id": 3, "name": "Local Quality", "description": "Fallback"},
            {"id": 4, "name": "Asymmetry", "description": "Fallback"},
            {"id": 5, "name": "Merging", "description": "Fallback"},
        ]
    except ImportError as e:
...
def run_fra_routing(problem: str) -> dict[str, Any]:
    try:
        ...
        return {
            "situation": fingerprint["situation"],
            "c4_state": str(fingerprint["c4_state"]),
            "recommended_operators": fingerprint["recommended_operators"],
            "situation_scores": fingerprint["scores"],
            "routed_chain": operators,
        }
...
def run_c4_observer(problem: str, c4_path: dict[str, Any]) -> dict[str, Any]:
    try:
        ...
        for pos in [ObserverPosition.IMMERSED, ObserverPosition.OBSERVING, ObserverPosition.META]:
            ...
            frames[pos.name] = {
                "visible_states_count": len(frame.visible_states),
                "blind_spots": frame.blind_spots,
...
async def search_isomorphisms(
    problem: str, papers: list[dict[str, Any]], triz_principles: list[str]
) -> dict[str, Any]:
    ...
    try:
        ...
        for iso in ISOMORPHISM_SEED:
            target_match = any(
                term in problem_lower for term in str(iso.get("target", "")).split("_")
            )  # type: ignore[attr-defined]
            source_match = any(
                term in problem_lower for term in str(iso.get("source", "")).split("_")
            )  # type: ignore[attr-defined]
            app_match = any(
                term in problem_lower for term in str(iso.get("applications", "")).split(",")
            )  # type: ignore[attr-defined]
                ...
                isomorphisms.append(
                    {
                        ...
                        "triz": iso.get("triz", []),
                        "applications": iso.get("applications", ""),
                    }
                )
            ...
            if term in problem.lower():
                try:
                    stored = await asyncio.wait_for(
                        core.search_isomorphisms(domain=term, limit=3), timeout=1.0
                    )
                    if stored:
...
def mine_contradictions(papers: list[dict[str, Any]]) -> dict[str, Any]:
    try:
        ...
        contradictions = (
            detector.find_all_contradictions(all_claims) if len(all_claims) >= 2 else []
        )
        return {
...
def build_temporal_kg(papers: list[dict[str, Any]], problem: str) -> dict[str, Any]:
    try:
        ...
        return {
            ...
            "stability": trajectory.get("stability", 0.0),
            "trend": trajectory.get("trend", "insufficient_data"),
            "paradigm_boundaries": len(boundaries),
        }
    except (ImportError, ModuleNotFoundError) as e:
...
def detect_paradigm_shift(papers: list[dict[str, Any]], domain: str) -> dict[str, Any]:
    try:
        ...
        return {
            ...
            "breakthrough_claims": breakthroughs[:3],
        }
    except (ImportError, ModuleNotFoundError) as e:
...
def run_strong_inference(problem: str, domain: str, hypothesis: dict[str, Any]) -> dict[str, Any]:
    try:
        ...
        return {
            ...
            "eliminated_hypotheses": len(result.eliminated_hypotheses),
            "explanation": result.explanation,
            "experiments_designed": len(result.experiments),
        }
    except (ImportError, ModuleNotFoundError) as e:
...
def run_abduction(problem: str, domain: str, papers: list[dict[str, Any]]) -> dict[str, Any]:
    try:
        ...
        result = engine.infer_to_best_explanation(
            observations=observations, domain=domain, max_hypotheses=4
        )
        return {
            "method": "Inference to the Best Explanation (Peirce)",
            "hypotheses_generated": len(result.hypotheses),
            "best_explanation": result.best_explanation.description[:200]
...
def run_falsification_engine(hypothesis: dict[str, Any], domain: str) -> dict[str, Any]:
    try:
        ...
        falsified, mt_reason = engine.apply_modus_tollens(
            first_sentence,
            "predicted outcome matches hypothesis",
            "observation differs from prediction",
        )
        return {
            "is_falsifiable": is_fals,
            "falsifiability_reason": reason,
            "demarcation": demarc,
            "modus_tollens_applied": falsified,
            "modus_tollens_reason": mt_reason,
        }
    except (ImportError, ModuleNotFoundError) as e:
...
def run_doe_design(domain: str) -> dict[str, Any]:
    try:
        ...
        config = DoEConfig(
            factors=[
                Factor(name="parameter_a", low=0.0, high=100.0, levels=2),
                Factor(name="parameter_b", low=0.0, high=100.0, levels=2),
            ],
        ...
        return {
            "design_type": result.design_type.name,
            "n_runs": result.design_matrix.shape[0],
            "n_factors": result.design_matrix.shape[1],
...
def run_reproducibility_check(problem: str) -> dict[str, Any]:
    try:
        ...
        validator.add_provenance(
            "discovery_pipeline",
            inputs={"problem": problem},
            source_file="src/api/v8_routers/discovery_v8.py",
        )
        report = validator.generate_report(check_results)
        return {
            "overall_score": round(report.overall_score, 2),
            "checks_passed": sum(
                1 for c in report.check_results if c.status.name == "PASS" and c.item.required
            ),
            "checks_total": len([c for c in report.check_results if c.item.required]),
        }
    except Exception as e:
    ...
    try:
        ...
        llm_text = await get_gateway().chat(
            messages=[
                {
                    "role": "user",
                    "content": "Generate a specific scientific hypothesis based on this problem. "
                    f"Problem: {_sanitize_for_prompt(problem)}\n"
                    f"Relevant papers:\n{paper_titles}\n"
                    "The hypothesis should: (1) be specific and falsifiable, (2) propose a novel mechanism or relationship, (3) predict measurable outcomes. Write 3-4 sentences. No markdown, no bullet points.",
                }
            ],
            max_tokens=300,
            temperature=0.4,
...
def run_causal_do_calculus(
    problem: str, domain: str, data: dict[str, Any] | None = None
) -> dict[str, Any]:
    ...
    try:
        ...
        if df is None or len(df) < 100:
            ...
            try:
                ...
                retrieved_df, retrieval_report = loop.run_until_complete(
                    asyncio.wait_for(task, timeout=45.0)
                )
            except RuntimeError:
                # No running loop
                retrieved_df, retrieval_report = asyncio.run(
                    orch.get_dataframe_for_hypothesis(problem, domain)
                )
        ...
        if df is not None and len(df) >= 100:
            graph = CausalDiscoveryEngine().discover(
                df, algorithm=data.get("algorithm", "pc") if data else "pc"
            )
...
def run_counterfactual(
    hypothesis: dict[str, Any], domain: str, data: dict[str, Any] | None = None
) -> dict[str, Any]:
    ...
    try:
        ...
        if df is None or len(df) < 100:
            ...
            try:
                ...
                task = asyncio.ensure_future(
                    orch.get_dataframe_for_hypothesis(hypothesis.get("text", ""), domain)
                )
                retrieved_df, retrieval_report = loop.run_until_complete(
                    asyncio.wait_for(task, timeout=45.0)
                )
        ...
        if df is not None and len(df) >= 100:
            graph = CausalDiscoveryEngine().discover(
                df, algorithm=(data or {}).get("algorithm", "pc")
            )
            gp_scm = GPSCM()
            gp_scm.fit(df, graph)
            ...
            intervention_var = (data or {}).get(
                "intervention", columns[0] if columns else "treatment"
            )
        ...
        toy_evidence: dict[str, float] = (
            {nodes[0]: 1.0, nodes[1]: 0.5} if len(nodes) > 1 else {nodes[0]: 1.0}
        )
        toy_result = engine.what_if(
            evidence=toy_evidence,
            intervention_target=nodes[0],
            intervention_value=0.0,
            target_variable=nodes[-1],
        )
        searched = retrieval_report.get("sources_count", 0)
        return {
...
async def generate_lean4_proof(hypothesis: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            "language": "lean4",
            "proof": f"(* Error generating proof: {e} *)",
            "generated": False,
            "error": str(e),
        }
...
def run_bayesian_model_averaging(
    hypothesis: dict[str, Any], monte_carlo: dict[str, Any]
) -> dict[str, Any]:
    try:
        ...
        return {
            ...
            "models_considered": len(result.models),
        }
    except Exception as e:
...
def run_bayesian_conjugate_update(monte_carlo: dict[str, Any]) -> dict[str, Any]:
    try:
        ...
        result = normal_normal(
            data=data, mu_prior=0.5, tau_prior=1.0 / 0.1, sigma_known=0.15, credible_level=0.95
        )
        return {
    ...
    try:
        ...
        evidence_list = [
            Evidence(
                ...
                description=p.get("title", "")[:100],
                citation_count=p.get("citations", 10)
                if isinstance(p.get("citations"), int)
                else 10,
                year=p.get("year", 2025) if isinstance(p.get("year"), int) else 2025,
            )
...
def run_empirical_validation(problem: str, c4_path: dict[str, Any]) -> dict[str, Any]:
    try:
        ...
        return {
            ...
            "validation_rate": round(summary.get("validation_rate", 0.0), 3),
            "calibration_status": summary.get("calibration", {}).get("status", "unknown"),
        }
    except Exception as e:
...
def generate_paper(
    ...
    for i, paper in enumerate(papers[:10]):
        ...
        authors_str = _escape_latex(
            " and ".join(authors_list) if isinstance(authors_list, list) else str(authors_list)
        )
        year = paper.get("year", 2025)
...
def run_cognitive_plugins(problem: str, hypothesis_text: str, domain: str) -> dict[str, Any]:
    ...
    plugins: list[tuple[str, str, Any]] = [
        ("swot", "SWOT Analysis", None),
        ("red_team", "Red Team Analysis", None),
        ("six_hats", "Six Thinking Hats", None),
        ("five_whys", "5 Whys", None),
        ("scamper", "SCAMPER", None),
        ("pareto", "Pareto Analysis", None),
        ("ishikawa", "Ishikawa/Fishbone", None),
        ("morphological", "Morphological Analysis", None),
        ("delphi", "Delphi Method", None),
        ("ooda", "OODA Loop", None),
        ("pre_mortem", "Pre-Mortem", None),
        ("second_order", "Second-Order Thinking", None),
        ("lateral_thinking", "Lateral Thinking", None),
        ("design_thinking", "Design Thinking", None),
        ("first_principles", "First Principles", None),
        ("inversion", "Inversion Thinking", None),
        ("constraint_relaxation", "Constraint Relaxation", None),
        ("analogical_reasoning", "Analogical Reasoning", None),
        ("bayesian_update", "Bayesian Update", None),
        ("persistence", "Persistence Analysis", None),
    ]
    ...
    try:
        result = await get_gateway().chat_json(
            messages=[
                {
                    "role": "user",
                    "content": "You are a skeptical Nature reviewer. Find 3 reasons why this is NOT a genuine discovery.\n\n"
                    f"HYPOTHESIS: {_sanitize_for_prompt(hypothesis[:500])}\n\n"
                    "CLOSEST EXISTING PAPERS:\n"
                    + "\n".join(f"- {t}" for t in closest_titles)
                    + "\n\n"
                    'Output JSON: {"reasons":["...","...","..."],'
                    '"recommendation":"PUBLISH"|"NEEDS_MORE_EVIDENCE"|"REJECT",'
                    '"explanation":"..."}',
                }
            ],
            max_tokens=500,
...
async def _refine_hypothesis_llm(
    problem,
    hypothesis,
    abort_reasons,
    top_papers,
    ...
    try:
        result = await get_gateway().chat_json(
            messages=[
                {
                    "role": "user",
                    "content": f"Refinement {iteration}/{max_iterations}. ORIGINAL PROBLEM: {_sanitize_for_prompt(problem, max_len=400)}\n"
                    ...
                    "Refine the hypothesis. Narrow, reframe, or intersect with a new concept. Consider these competing framings and pivot if one is superior. "
                    'Output JSON: {"strategy":"narrow|reframe|intersect|shift|scale",'
                    '"refined_problem":"...","refined_hypothesis":"...",'
                    '"why_better":"...","no_improvement":false}',
                }
            ],
            temperature=0.6,
            max_tokens=600,
    ...
    return {
        ...
        "refined_hypothesis": hypothesis[:1000],
        "why_better": "LLM refinement unavailable",
        "no_improvement": True,
        "_fallback": True,
    }
...
def _build_dissertation(discovery: dict, attempts: list) -> dict:
    ...
    output_mode = (
        thresholds.get("output_mode", "human") if isinstance(thresholds, dict) else "human"
    )
  ...
  year    = {{{year}}}{doi_line},
    ...
    lit_review_parts = [
        f"A comprehensive search across {sources} databases returned {papers_found} relevant publications."
    ]
    ...
    if gaps_found > 0:
        lit_review_parts.append(
            f"Gap analysis identified {gaps_found} unresolved research gaps across the reviewed literature."
        )
    if contradictions > 0:
    ...
    sections = [
        {
            ...
            "content": f"{hyp_text[:300]}" if hyp_text else "Introduction pending.",
        },
        {"heading": "2. Literature Review", "content": lit_review},
        {
            "heading": "3. Hypothesis",
            "content": f"{hyp_text[:1000]}" if hyp_text else "See hypothesis section.",
        },
        {
            "heading": "4. Methodology",
            "content": f"Computational discovery pipeline integrating multi-source knowledge acquisition ({papers_found} papers from {sources} databases [1-{len(key_papers)}]), contradiction mining ({contradictions} contradictions), and formal verification.",
        },
        {
            "heading": "5. Predictions",
            "content": "Falsifiable predictions derived from hypothesis generation and gap analysis.",
        },
        {
            "heading": "6. Validation",
            "content": f"Self-critique: {discovery.get('self_critique', {}).get('recommendation', 'N/A')}."
            if discovery.get("self_critique")
            else "Validation pending.",
        },
        {
            "heading": "7. Implications",
            "content": "Implications for theory and practice — see discussion.",
        },
        {
            "heading": "8. Conclusion",
            "content": f"{hyp_text[:300]}" if hyp_text else "Conclusion pending.",
        },
