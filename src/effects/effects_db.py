...
class PhysicalEffect:
    ...
    applications: list[str] = None  # type: ignore[assignment]

    def __post_init__(self) -> None:
        if self.parameters is None:
            self.parameters = []  # type: ignore[unreachable]
        if self.applications is None:
            self.applications = []  # type: ignore[unreachable]
...
def get_effects_database() -> EffectsDatabase:
    """Get singleton effects database (backed by DI container)."""
    from src.di.container import get_container
    return get_container().get_or_register("effects_database", EffectsDatabase)
