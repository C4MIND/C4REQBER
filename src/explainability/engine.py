"""
TURBO-CDI: Explainability Engine
...
class ExplainabilityEngine:
    ...
    def _explain_step(
        ...
        return StepExplanation(
            ...
            to_state=to_state,
            what=exp_data.get("what", f"Apply {operator}"),
            why=exp_data.get("why", "Transforms the problem"),
            how=exp_data.get("how", f"Execute {operator}"),
            example=exp_data.get("example", ""),
            confidence=0.8,
            related_principles=op_data.get("related", []),
        )

    def _determine_states(self, operator: str) -> tuple:
