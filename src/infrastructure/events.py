"""Event Bridge — cross-cutting pub/sub event bus for real-time streaming.

Lives in `infrastructure` (not `pipeline`) so any layer can publish/subscribe
without depending on the high-level pipeline package.

Usage:
    from src.infrastructure.events import event_bus

    # In a pipeline / verifier / etc.:
