...
class CrossrefAdapter(BaseP6SourceAdapter):
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        headers.setdefault("User-Agent", "c44tcdi (mailto:c44tcdi@example.com)")
    ...
    def _normalize(self, data: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for item in data:
            ...
            for a in item.get("author", []):
                ...
                if name:
                    authors.append(name)
            date_parts = ((item.get("published-print") or item.get("published-online") or item.get("published") or {}).get("date-parts") or [[0]])
