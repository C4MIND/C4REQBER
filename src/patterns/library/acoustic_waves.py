...
class AcousticWavesConfig(BaseConfig):
    ...
    # Time stepping
    dt: float = None  # type: ignore  # Computed from CFL
    n_steps: int = 500
    cfl: float = 0.5
    ...
    def __post_init__(self) -> None:
        if self.dt is None:
            dx = self.Lx / self.nx  # type: ignore[unreachable]
...
class AcousticWaves(BasePattern, GPUMixin):
    ...
    def __init__(self, config: AcousticWavesConfig | None = None) -> None:
        BasePattern.__init__(self, config or AcousticWavesConfig())
        GPUMixin.__init__(self)
        self.config: AcousticWavesConfig = self.config

        # Fields
        self.p: np.ndarray = None  # type: ignore  # Pressure
        self.vx: np.ndarray = None  # type: ignore  # x-velocity
        self.vy: np.ndarray = None  # type: ignore  # y-velocity
        self.vz: np.ndarray = None  # type: ignore  # z-velocity (3D only)

        # PML damping
        self.sigma_x: np.ndarray = None  # type: ignore[assignment]
        self.sigma_y: np.ndarray = None  # type: ignore[assignment]
        self.sigma_z: np.ndarray = None  # type: ignore[assignment]

        # Spectral operators
        self.kx: np.ndarray = None  # type: ignore[assignment]
        self.ky: np.ndarray = None  # type: ignore[assignment]
        self.kz: np.ndarray = None  # type: ignore[assignment]
    ...
    def _fd_divergence(
        ...
        return div  # type: ignore[no-any-return]

    def _add_source(self, step: int) -> None:
        """Add source term."""
        nx, ny, nz = self.config.nx, self.config.ny, self.config.nz
        sx, sy, sz = self.config.source_position
    ...
    def _step_pseudospectral(self) -> None:
    ...
    def _compute_energy(self) -> tuple[float, float]:
        ...
        # Use float64 and norm for numerical stability against overflow
        p_f64 = self.p.astype(np.float64)
        p_norm = np.linalg.norm(p_f64)
        pe = (p_norm * p_norm) / (2 * self.config.rho0 * self.config.c0**2)
        pe *= self.dx * self.dy
        if self.config.nz > 1:
            pe *= self.dz
    ...
    def run(self, hypothesis: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        # Stable RMS computation avoiding overflow with large pressures
        p_max = np.max(np.abs(self.p))
        if p_max > 0:
            rms_pressure = p_max * np.sqrt(np.mean((self.p / p_max) ** 2))
        else:
            rms_pressure = 0.0
