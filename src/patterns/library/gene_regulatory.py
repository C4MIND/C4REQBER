...
class GeneRegulatoryPattern(SimulationPattern):
    ...
    def __init__(self) -> None:
        ...
        self.regulation_types: np.ndarray | None = None
    ...
    async def _boolean_simulation(self) -> dict[str, Any]:
        ...
        def update_rule(s: np.ndarray) -> np.ndarray:
            ...
            for i in range(n):
                ...
                total = np.sum(self.regulation_types[:, i] * s)  # type: ignore[index]
                # Threshold at 0
        ...
        for step in range(cfg.num_steps):
            if cfg.update_mode == "synchronous":
                ...
                total = np.sum(self.regulation_types[:, i] * state)  # type: ignore[index]
                new_state[i] = 1 if total > 0 else 0
        ...
        trajectory = np.array(trajectory)  # type: ignore[assignment]
        ...
        metrics = {
            "num_genes": n,
            "num_edges": int(np.sum(self.adjacency)),  # type: ignore[arg-type]
        ...
        logs = [
            "Boolean network simulation completed",
        ...
        return {
            "metrics": metrics,
            "logs": logs,
            "trajectory": trajectory.tolist(),  # type: ignore[attr-defined]
    ...
    def _find_all_attractors(self) -> list[tuple]:
        ...
        def state_to_tuple(s: Any) -> Any:
            return tuple(s)

        def update(s: Any) -> Any:
            """Update."""
    ...
    async def _ode_simulation(self) -> dict[str, Any]:
        ...
        def grn_odes(t: Any, y: Any) -> Any:
            ...
            for i in range(n):
                ...
                regulation = 0
                for j in range(n):
                    if self.adjacency[j, i]:  # type: ignore[index]
                        reg = self.regulation_types[j, i]  # type: ignore[index]
                        if reg > 0:  # Activation
                            regulation += self._hill_activation(y[j])  # type: ignore[assignment]
                        else:  # Repression
                            regulation += self._hill_repression(y[j])  # type: ignore[assignment]

                # Production minus degradation
                production = cfg.alpha * regulation / max(1, np.sum(self.adjacency[:, i]))  # type: ignore[index]
        ...
        logs = [
            "ODE gene regulatory simulation completed",
    ...
    def _hill_activation(self, x: float) -> float:
        ...
        return x**self.config.hill_n / (self.config.theta**self.config.hill_n + x**self.config.hill_n)  # type: ignore[no-any-return]

    def _hill_repression(self, x: float) -> float:
        """Hill function for repression"""
        return self.config.theta**self.config.hill_n / (self.config.theta**self.config.hill_n + x**self.config.hill_n)  # type: ignore[no-any-return]
    ...
    async def _hybrid_simulation(self) -> dict[str, Any]:
        ...
        def hybrid_odes(t: Any, y: Any) -> Any:
            """Hybrid odes."""
            dydt = np.zeros(n)
            ...
            for i in range(n):
                # Boolean update rule
                total = np.sum(self.regulation_types[:, i] * bool_state)  # type: ignore[index]
        ...
        logs = [
            "Hybrid GRN simulation completed",
    ...
    async def _threshold_simulation(self) -> dict[str, Any]:
        ...
        def threshold_odes(t: Any, y: Any) -> Any:
            """Threshold odes."""
            dydt = np.zeros(n)
            bool_state = (y > cfg.theta).astype(int)
