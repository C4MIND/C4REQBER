...
class HeterogeneousAgentsModel:
    ...
    def __init__(self, config: HeterogeneousAgentsConfig = None) -> None:  # type: ignore[assignment]
    ...
    def solve_individual_problem(self, w: float, r: float,
        ...
        for e in range(2):  # employment states
            ...
            for i_a, a in enumerate(self.a_grid):
                ...
                # Find optimal a'
                def objective(a_next: Any, _resources: float = float(resources)) -> Any:
                    """Objective."""
                    if a_next < 0 or a_next > _resources:
                        return -1e10
                    c = _resources - a_next
                    if c <= 0:
                        return -1e10
                    ...
                    v_next = np.interp(a_next, self.a_grid, value_func[:, e])  # noqa: B023
...
class TestHeterogeneousAgentsModel(unittest.TestCase):
    ...
    def test_transition_matrices(self) -> None:
    ...
    def test_simulation_structure(self) -> None:
