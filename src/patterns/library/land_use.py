"""
C4REQBER v6.0 - Land Use Pattern
...
class LandUseConfig:
    ...
    # Accessibility
    zone_centroids: np.ndarray | None = None  # Shape: (n_zones, 2)
    transport_network: np.ndarray | None = None  # Distance matrix
    ...
    # Zoning
    zoning: dict[int, list[str]] | None = None  # Zone -> allowed uses
...
class LandUsePattern:
    ...
    def __init__(self, config: LandUseConfig | None = None) -> None:
        self.config = config or LandUseConfig()
        self.allocation: np.ndarray | None = None  # Shape: (n_zones, n_types)
        self.utilities: np.ndarray | None = None
    ...
    def _calculate_accessibility(self, zone: int, lu_type: str) -> float:
        ...
        for j in range(cfg.n_zones):
            dist = cfg.transport_network[zone, j]  # type: ignore[index]
            if dist > 0:
    ...
    def _calculate_agglomeration(self, zone: int, lu_type_idx: int) -> float:
        ...
        for j in range(cfg.n_zones):
            ...
            if dist < 3:  # Within interaction radius
                agglomeration += self.allocation[j, lu_type_idx] / (1 + dist)  # type: ignore[index]
    ...
    def _calculate_competition(self, zone: int, lu_type_idx: int) -> float:
        ...
        if total_allocation > capacity:
            return -(total_allocation - capacity) / capacity  # type: ignore[no-any-return]
        return 0.0
    ...
    def _calculate_utility(self, zone: int, lu_type_idx: int) -> float:
        ...
        return utility  # type: ignore[no-any-return]

    def _update_allocation(self) -> Any:
    ...
    def _format_output(
        ...
        type_by_zone = {}
        for t, lu_type in enumerate(cfg.land_use_types):
            type_totals[lu_type] = float(np.sum(self.allocation[:, t]))  # type: ignore[index]
            type_by_zone[lu_type] = self.allocation[:, t].tolist()  # type: ignore[index]

        # Dominant land use per zone
        dominant_types = np.argmax(self.allocation, axis=1)  # type: ignore[arg-type]
        ...
        concentration = np.sum(self.allocation**2, axis=1) / (  # type: ignore[operator]
            np.sum(self.allocation, axis=1) ** 2 + 1e-10  # type: ignore[arg-type]
        )
        ...
        return {
            "allocation": self.allocation.tolist(),  # type: ignore[union-attr]
            ...
            "zoning_compliance": float(
                np.sum(self.allocation * self.zoning_matrix) / np.sum(self.allocation)  # type: ignore[arg-type, operator]
            ),
            "config": {
                "n_zones": cfg.n_zones,
