...
class MonteCarloPattern(SimulationPattern):
    ...
    def _build_model(
        ...
        """
        # Build model from hypothesis parameters
        # Future: LLM-powered model generation from natural language
    ...
    async def _importance_sampling(
        ...
        for batch in range(0, config.n_samples, config.batch_size):
            ...
            # Product of per-dimension PDFs (sum in log-space)
            from scipy.stats import norm

            log_gaussian_pdf = norm.logpdf(inputs, proposal_mean, proposal_std).sum(axis=1)
            log_uniform_pdf = np.full_like(log_gaussian_pdf, np.log(1.0 / 10.0))
            batch_weights = np.exp(log_uniform_pdf - log_gaussian_pdf)
        ...
        # Weighted average
        results = np.array(results)  # type: ignore[assignment]
        weights = np.array(weights)  # type: ignore[assignment]
        weighted_results = results * weights / weights.sum() * len(results)  # type: ignore[attr-defined, operator]

        return weighted_results  # type: ignore[no-any-return]
    ...
    async def _sobol_sampling(
        ...
        for batch in range(0, config.n_samples, config.batch_size):
            ...
            # Generate Sobol samples — round to power of 2 to satisfy balance properties
            sobol_n = 2 ** int(np.ceil(np.log2(max(batch_size, 2))))
            inputs = sampler.random(sobol_n)[:batch_size]
    ...
    def _calculate_confidence(
        ...
        return min(0.95, confidence)  # type: ignore  # Cap at 0.95 (Monte Carlo limit)
