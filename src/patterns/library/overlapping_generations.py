...
class OverlappingGenerationsModel:
    ...
    def __init__(self, config: OLGConfig = None) -> None:  # type: ignore[assignment]
    ...
    def _solve_savings_rate(self, w: float, r: float) -> float:
        ...
        try:
            ...
            return np.clip(s_opt / w, 0.01, 0.99)  # type: ignore[no-any-return]
        except (ValueError, RuntimeError):
            return beta / (1 + beta)
    ...
    def _compute_generational_accounts(self) -> dict[str, list[float]]:
        ...
        return {"generations": generations, "net_accounts": accounts}  # type: ignore[dict-item]
...
class TestOLGModel(unittest.TestCase):
    """TestOLGModel."""
    def test_steady_state_positive(self) -> None:
    ...
    def test_transition_convergence(self) -> None:
    ...
    def test_factor_price_relationships(self) -> None:
    ...
    def test_generational_accounts(self) -> None:
