"""
C4REQBER v6.0 - Rumor Spreading Pattern
...
class RumorConfig:
    ...
    # Counter-rumor
    counter_rumor_start: float | None = None  # When to introduce counter-rumor
...
class RumorSpreadingPattern:
    ...
    def __init__(self, config: RumorConfig | None = None) -> None:
    ...
    def _build_network(self) -> np.ndarray:
        ...
        if cfg.network_type == "complete":
            ...
            adj = np.random.random((n, n)) < cfg.network_param  # type: ignore[assignment]
    ...
    def _get_neighbors(self, i: int) -> list[int]:
        ...
        return list(np.where(self.network[i] == 1)[0])  # type: ignore[index]
    ...
    def _sir_step(self) -> tuple[int, int, int]:
        ...
        new_states = self.states.copy()  # type: ignore[union-attr]

        for i in range(n):
            if self.states[i] == 2:  # type: ignore  # Spreader
                # Contact neighbors
                neighbors = self._get_neighbors(i)
                for j in neighbors:
                    if self.states[j] == 0:  # type: ignore  # Ignorant
            ...
            elif self.states[i] == 3:  # type: ignore  # Stifler
                # Can forget too
                if np.random.random() < cfg.forgetting_rate * cfg.dt:
                    new_states[i] = 0

        self.states = new_states
        return self._count_states()  # type: ignore[return-value]
    ...
    def _seir_step(self) -> tuple[int, int, int]:
        ...
        for i in range(n):
            ...
            elif self.states[i] == 1:  # type: ignore  # Exposed
    ...
    def _seiz_step(self) -> tuple[int, int, int]:
        ...
        for i in range(n):
            if self.states[i] == 2:  # type: ignore  # Spreader
                ...
                for j in neighbors:
                    if self.states[j] == 0:  # type: ignore  # Ignorant
                        if np.random.random() < cfg.spreading_rate * cfg.dt:
                            new_states[j] = 2
                    elif self.states[j] == 4:  # type: ignore  # Skeptic - might become spreader
            ...
            elif self.states[i] == 0:  # type: ignore  # Ignorant can become skeptic
                neighbors = self._get_neighbors(i)
                skeptics_near = sum(1 for j in neighbors if self.states[j] == 4)  # type: ignore[index, misc]
                if (
                    skeptics_near > 0
    ...
    def _count_states(self) -> tuple[int, int, int, int, int]:
        ...
        counts = [0, 0, 0, 0, 0]
        for s in self.states:  # type: ignore[union-attr]
            counts[s] += 1
        return tuple(counts)  # type: ignore[return-value]

    def _record_state(self, time: float) -> None:
    ...
    def _format_output(self) -> dict[str, Any]:
        ...
        return {
            ...
            "network": {
                ...
                "density": float(
                    np.sum(self.network) / (cfg.n_agents * (cfg.n_agents - 1))  # type: ignore[arg-type]
                ),
            },
            "config": {
