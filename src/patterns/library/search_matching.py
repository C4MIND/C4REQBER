...
class SearchMatchingModel:
    ...
    def __init__(self, config: SearchMatchingConfig = None) -> None:  # type: ignore[assignment]
    ...
    def matching_function(self, u: float, v: float) -> float:
        ...
        if u <= 0 or v <= 0:
            return 0
        return cfg.eta * (u ** cfg.alpha) * (v ** (1 - cfg.alpha))  # type: ignore[no-any-return]
    ...
    def job_finding_rate(self, theta: float) -> float:
        ...
        """
        cfg = self.config
        return cfg.eta * (theta ** (1 - cfg.alpha))  # type: ignore[no-any-return]
    ...
    def job_filling_rate(self, theta: float) -> float:
        ...
        if theta <= 0:
            return float('inf')
        return cfg.eta * (theta ** (-cfg.alpha))  # type: ignore[no-any-return]
    ...
    def solve_steady_state(self) -> dict[str, float]:
        ...
        def equilibrium_condition(theta: Any) -> Any:
    ...
    def simulate_dynamics(self, T: int = 100, shock_period: int = 20) -> dict[str, list[float]]:
        ...
        for t in range(T - 1):
            ...
            def eqm_cond(th: Any) -> Any:
                """Eqm cond."""
                if th <= 0:
                    return float('inf')
                q = self.job_filling_rate(th)
                w_t = cfg.gamma * (p + cfg.c * th) + (1 - cfg.gamma) * cfg.z  # noqa: B023
                J = (p - w_t) / (cfg.r + cfg.delta)  # noqa: B023
                return cfg.c - q * J
...
class TestSearchMatchingModel(unittest.TestCase):
    ...
    """TestSearchMatchingModel."""
    def test_matching_function(self) -> None:
    ...
    def test_job_finding_rate(self) -> None:
    ...
    def test_steady_state_properties(self) -> None:
    ...
    def test_beveridge_curve(self) -> None:
    ...
    def test_policy_analysis(self) -> None:
