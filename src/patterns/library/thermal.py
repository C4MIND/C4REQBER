...
class ThermalPattern(SimulationPattern):
    ...
    async def run(  # type: ignore[override]
        ...
        """Execute thermal simulation with Newton (or fallback)."""
        ...
        if bridge.available:
            newton_config = {
                ...
                "thermal_conductivity": config.get("thermal_conductivity", 200.0),
                "density": config.get("density", 2700.0),
                "specific_heat": config.get("specific_heat", 900.0),
                "initial_temp": config.get("initial_temp", 20.0),
        ...
        logger.info(f"Starting thermal simulation {simulation_id} (legacy)")
