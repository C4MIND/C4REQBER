"""
Base classes and mixins for TURBO-CDI patterns.
"""
...
class GPUMixin:
    ...
    def to_gpu(self, arr: np.ndarray) -> Any:
        ...
        if not self._gpu_available:
            return arr
        return self._cuda.to_device(arr)
    
    def to_cpu(self, gpu_arr: Any) -> np.ndarray:
        """Transfer data from GPU to CPU memory."""
        if not self._gpu_available:
            return gpu_arr
        return gpu_arr.copy_to_host()
    
    def gpu_parallel(self, func, blockspergrid: Tuple, threadsperblock: Tuple, *args):
...
class BasePattern(ABC):
    """Abstract base class for all TURBO-CDI patterns."""
