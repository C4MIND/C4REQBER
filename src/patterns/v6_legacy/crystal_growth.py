"""
TURBO-CDI v6.0 - Crystal Growth Pattern
...
class CrystalGrowthPattern:
    ...
    def __init__(self, config: Optional[CrystalGrowthConfig] = None):
        self.config = config or CrystalGrowthConfig()

        # Fields
        self.phi: Optional[np.ndarray] = None  # Phase field (liquid=-1, solid=+1)
        self.T: Optional[np.ndarray] = None  # Temperature field
    ...
    def _create_nuclei(self):
        ...
        for _ in range(cfg.n_nuclei):
            ...
            self.phi[r <= cfg.nucleation_radius] = np.tanh(
    ...
    def _anisotropy_function(self, theta: np.ndarray) -> np.ndarray:
        ...
        if cfg.symmetry == CrystalSymmetry.ISOTROPIC:
            ...
            # 4-fold symmetry
            return 1.0 + cfg.anisotropy_strength * np.cos(cfg.anisotropy_mode * theta)

        elif cfg.symmetry == CrystalSymmetry.HEXAGONAL:
            # 6-fold symmetry
            return 1.0 + cfg.anisotropy_strength * np.cos(6 * theta)

        elif cfg.symmetry == CrystalSymmetry.TETRAGONAL:
            # 4-fold with different strength
            return 1.0 + cfg.anisotropy_strength * np.cos(4 * theta)
    ...
    def _phase_field_rhs(self) -> np.ndarray:
        ...
        thermal_force = -lambda_pf * (1 - phi**2) ** 2 * (T - cfg.T_melt) / cfg.T_melt

        # Anisotropic diffusion term
        diffusion = cfg.epsilon**2 * self._anisotropic_laplacian(phi)
        ...
        if cfg.thermal_noise:
            interface_mask = np.abs(phi) < 0.9  # Only at interface
            noise = cfg.noise_amplitude * np.random.randn(*phi.shape) * interface_mask
            rhs += noise
    ...
    def _thermal_rhs(self) -> np.ndarray:
        ...
        # Thermal diffusion
        diffusion = cfg.alpha * self._laplacian(self.T)
    ...
    def _compute_interface_length(self) -> float:
        ...
        threshold = 0.5
        interface = np.abs(self.phi) < threshold
    ...
    def _format_output(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "final_phi": self.phi.tolist(),
            "final_temperature": self.T.tolist(),
