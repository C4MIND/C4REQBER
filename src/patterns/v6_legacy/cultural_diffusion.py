"""
TURBO-CDI v6.0 - Cultural Diffusion Pattern
...
class CulturalDiffusionPattern:
    ...
    def __init__(self, config: Optional[CulturalDiffusionConfig] = None):
        self.config = config or CulturalDiffusionConfig()
        self.culture: Optional[np.ndarray] = None  # Shape: (n_agents, n_features)
        self.positions: Optional[np.ndarray] = None  # Shape: (n_agents, 2)
    ...
    def _initialize(self):
        ...
        if cfg.grid_size[0] * cfg.grid_size[1] != cfg.n_agents:
            ...
            side = int(np.sqrt(cfg.n_agents))
            cfg.grid_size = (side, side)
            cfg.n_agents = side * side
    ...
    def _cultural_similarity(self, i: int, j: int) -> float:
        ...
        shared = np.sum(self.culture[i] == self.culture[j])
        return shared / self.config.n_features
    ...
    def _interact(self, i: int, j: int) -> bool:
        ...
        if np.random.random() < similarity:
            ...
            diff_features = np.where(self.culture[i] != self.culture[j])[0]
            if len(diff_features) > 0:
                feature = np.random.choice(diff_features)
                # i adopts j's trait
                self.culture[i, feature] = self.culture[j, feature]
                return True
    ...
    def _mutate(self):
        ...
        for _ in range(n_mutations):
            ...
            self.culture[agent, feature] = np.random.randint(cfg.n_traits)
        ...
        for start in range(cfg.n_agents):
            ...
            while queue:
                ...
                for neighbor in self.neighbors[current]:
                    if np.array_equal(self.culture[current], self.culture[neighbor]):
    ...
    def _cultural_diversity(self) -> float:
        ...
        unique_cultures = set()
        for i in range(cfg.n_agents):
            culture_tuple = tuple(self.culture[i])
    ...
    def _format_output(
        ...
        for f in range(cfg.n_features):
            trait_counts = np.bincount(self.culture[:, f], minlength=cfg.n_traits)
        ...
        return {
            ...
            "final_culture": self.culture.tolist(),
            "history": [
                {
                    "step": h["step"],
