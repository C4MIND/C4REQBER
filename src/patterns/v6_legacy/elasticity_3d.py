...
class HexahedralElement:
    ...
    def _compute_volume(self) -> float:
        ...
        vol = 0.0
        for xi, w in zip(self.gauss_points, self.gauss_weights):
            dN = self.shape_function_derivatives_q1(xi)
            J = dN.T @ self.nodes
...
class Elasticity3D(BasePattern):
    ...
    def __init__(self, config: Optional[Elasticity3DConfig] = None):
    ...
    def _assemble_system(self) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        ...
        for e_idx, element in enumerate(self.elements):
            ...
            for xi, w in zip(element.gauss_points, element.gauss_weights):
                B, detJ = element.compute_b_matrix(xi)
    ...
    def run(self, hypothesis: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
        Execute 3D elasticity simulation.
