...
class GravityTradeModel:
    ...
    def __init__(self, config: GravityTradeConfig = None):
        self.config = config or GravityTradeConfig()
        np.random.seed(self.config.random_seed)
        self.countries: List[str] = []
        self.productivity: np.ndarray = None
        self.labor: np.ndarray = None
        self.wages: np.ndarray = None
        self.trade_costs: np.ndarray = None
    ...
    def compute_welfare_gains(self, counterfactual_costs: np.ndarray = None) -> Dict[str, float]:
        """
        ...
        if counterfactual_costs is not None:
            ...
            home_consumption_share = np.diag(X_base) / X_base.sum(axis=1)
...
class TestGravityTradeModel(unittest.TestCase):
    ...
    def test_trade_costs_diagonal(self):
    ...
    def test_welfare_gains(self):
        ...
        self.assertEqual(len(welfare["welfare_gains_pct"]), 5)
        
        # Welfare gains should be positive
        self.assertTrue(all(g > 0 for g in welfare["welfare_gains_pct"]))
    ...
    def test_trade_elasticity(self):
