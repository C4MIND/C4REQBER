...
class HodgkinHuxleyPattern(SimulationPattern):
    ...
    async def _simulate_hh(self) -> Dict[str, Any]:
        ...
        logs = [
            f"Hodgkin-Huxley simulation completed",
    ...
    def _get_stimulus(self, t: float) -> float:
        ...
        if cfg.stim_type == StimulusType.STEP:
            ...
            freq = 0.5  # kHz
            return cfg.I_inj * np.sin(2 * np.pi * freq * (t - cfg.stim_start))
    ...
    def _calculate_metrics(
        ...
        # Current integrals
        total_Na_current = float(np.trapz(I_Na, t))
        total_K_current = float(np.trapz(I_K, t))
