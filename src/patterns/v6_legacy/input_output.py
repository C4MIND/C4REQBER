...
class InputOutputConfig:
    ...
    intermediate_demand: np.ndarray = None
...
class InputOutputModel:
    ...
    def __init__(self, config: InputOutputConfig = None):
    ...
    def _setup_data(self):
        ...
        if self.config.total_output is None:
            ...
            self.total_output = np.array([500, 800, 600, 400, 300], dtype=float)
        else:
            self.total_output = self.config.total_output.astype(float)
        ...
        if self.config.intermediate_demand is None:
            ...
            self.tech_coeffs = self.config.intermediate_demand

        if self.config.final_demand is None:
            self.final_demand = np.array([150, 200, 180, 100, 80], dtype=float)
        else:
            self.final_demand = self.config.final_demand.astype(float)
        ...
        if self.config.value_added is None:
            ...
            self.value_added = self.config.value_added.astype(float)
    ...
    def run(self) -> Dict[str, Any]:
        ...
        Z = self.tech_coeffs * self.total_output[np.newaxis, :]
        ...
        for i in range(n):
            for j in range(n):
                if self.total_output[j] > 0:
                    A[i, j] = Z[i, j] / self.total_output[j]
        ...
        # Income multipliers
        income_coeffs = self.value_added / self.total_output
        ...
        return {
            ...
            "final_demand": self.final_demand.tolist(),
            "value_added": self.value_added.tolist(),
...
class TestInputOutputModel(unittest.TestCase):
    def test_model_initialization(self):
        ...
        self.assertEqual(len(model.total_output), 5)
