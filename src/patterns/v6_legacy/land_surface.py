"""
TURBO-CDI v6.0 - Land Surface Pattern
...
class LandSurfacePattern:
    ...
    def __init__(self, config: Optional[LandSurfaceConfig] = None):
    ...
    def _soil_moisture_update(self, precip: np.ndarray, ET: np.ndarray):
    ...
    def _temperature_update(self, Q_g: np.ndarray):
    ...
    def _snow_update(self, precip: np.ndarray, T_atm: np.ndarray):
    ...
    def _step(self, day: float, hour: float):
...
class TestLandSurface(unittest.TestCase):
    ...
    def test_soil_moisture_update(self):
