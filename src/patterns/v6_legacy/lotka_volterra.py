...
class LotkaVolterraConfig:
    ...
    t_max: float = 100.0
    dt: float = 0.01
    initial_populations: Optional[List[float]] = None
...
class LotkaVolterraPattern(SimulationPattern):
    ...
    def __init__(self):
        ...
        self.config: Optional[LotkaVolterraConfig] = None
        self.solution = None
    ...
    async def _simulate(self, hypothesis: Hypothesis) -> Dict[str, Any]:
        ...
        y0 = np.array(self.config.initial_populations)

        # Set up ODE
        if self.config.model_type == "predator_prey":
        ...
        elif self.config.model_type == "competitive":
            # Competitive Lotka-Volterra
            r = np.array(self.config.growth_rates)
            K = hypothesis.parameters.get("carrying_capacities", [100.0] * self.config.n_species)
            K = np.array(K)

            # Competition matrix
            alpha = hypothesis.parameters.get("competition_matrix")
            if alpha is None:
                alpha = np.ones((self.config.n_species, self.config.n_species))
            ...
            if beta is None:
                beta = 0.1 * np.ones((self.config.n_species, self.config.n_species))
    ...
    def _analyze_results(self) -> Dict[str, Any]:
        ...
        logs = [
            f"Lotka-Volterra ({self.config.model_type}): {n_species} species",
