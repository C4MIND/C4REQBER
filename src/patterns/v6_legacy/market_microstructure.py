...
class Agent:
    ...
    def generate_order(self, book: OrderBook, fundamental: float, time: float) -> Optional[Order]:
...
class NoiseTrader(Agent):
    ...
    def __init__(self, agent_id: int, volatility: float = 0.02):
...
class MarketMaker(Agent):
    ...
    def __init__(self, agent_id: int, spread_target: float = 0.02):
        super().__init__(agent_id)
...
class MarketMicrostructureModel:
    ...
    def __init__(self, config: MarketMicrostructureConfig = None):
