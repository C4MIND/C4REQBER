"""
TURBO-CDI v6.0 - Migration Pattern
...
class MigrationConfig:
    ...
    # Regions
    n_regions: int = 10
    region_populations: Optional[np.ndarray] = None
    region_attractiveness: Optional[np.ndarray] = None

    # Distance matrix
    distance_matrix: Optional[np.ndarray] = None
    region_coordinates: Optional[np.ndarray] = None  # For distance calculation
...
class MigrationPattern:
    ...
    def __init__(self, config: Optional[MigrationConfig] = None):
        ...
        self.flows: Optional[np.ndarray] = None
    ...
    def _gravity_flows(self) -> np.ndarray:
        ...
        for i in range(n):
            for j in range(n):
                ...
                mass_term = (self.population[i] ** cfg.alpha) * (
                    self.population[j] ** cfg.beta * cfg.region_attractiveness[j]
                )
                distance_term = cfg.distance_matrix[i, j] ** (-cfg.gamma)

                flows[i, j] = mass_term * distance_term

        # Normalize to total migration
        total_migrants = np.sum(self.population) * cfg.migration_rate
    ...
    def _radiation_flows(self) -> np.ndarray:
        ...
        for i in range(n):
            for j in range(n):
                ...
                radius = cfg.distance_matrix[i, j]
                intervening_pop = 0
                for k in range(n):
                    if k != i and k != j:
                        if cfg.distance_matrix[i, k] < radius:
                            intervening_pop += self.population[k]

                # Radiation model probability
                m_i = self.population[i]
                m_j = self.population[j] * cfg.region_attractiveness[j]

                if m_i + intervening_pop > 0:
                    prob = (m_i * m_j) / (
                        (m_i + intervening_pop) * (m_i + intervening_pop + m_j)
                    )
                    flows[i, j] = prob * self.population[i] * cfg.migration_rate

        return flows

    def _update_populations(self, flows: np.ndarray):
    ...
    def _calculate_migration_rate(self) -> float:
        ...
        total_movers = np.sum(np.abs(self.population - self.initial_population))
        return total_movers / np.sum(self.initial_population)

    def _calculate_concentration(self) -> float:
        """Calculate population concentration (Herfindahl index)"""
        shares = self.population / np.sum(self.population)
    ...
    def _format_output(self, all_flows: List[np.ndarray]) -> Dict[str, Any]:
        ...
        net_migration = self.population - self.initial_population

        # Migration connectivity
        connectivity = np.sum(self.flows > 0) / (cfg.n_regions * (cfg.n_regions - 1))

        # Gini coefficient of population distribution
        gini = self._calculate_gini(self.population)
            ...
            "region_data": [
                {
                    ...
                    "final": float(self.population[i]),
                    "net_migration": float(net_migration[i]),
                    "attractiveness": float(cfg.region_attractiveness[i]),
                }
