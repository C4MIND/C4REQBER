"""
TURBO-CDI v6.0 - Molecular Dynamics Pattern
...
class MolecularDynamicsPattern:
    ...
    def _calculate_temperature(self) -> float:
        ...
        ke = 0.5 * mass * np.sum(self.velocities**2)
        # T = 2*KE / (3*N*kB)
        return 2 * ke / (3 * cfg.n_atoms * cfg.kB)
    ...
    def _calculate_kinetic_energy(self) -> float:
        ...
        mass = 39.948
        return 0.5 * mass * np.sum(self.velocities**2)
    ...
    def _velocity_verlet_step(self):
        ...
        self.velocities += self.forces * cfg.dt / (2 * mass)

        # r(t + dt) = r(t) + v(t + dt/2)*dt
        self.positions += self.velocities * cfg.dt

        # Apply periodic boundary conditions
        self.positions -= cfg.box_size * np.floor(self.positions / cfg.box_size)
