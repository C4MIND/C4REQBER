...
class OctreeNode:
    ...
    def __init__(self, center: np.ndarray, size: float):
        ...
        self.children = [None] * 8  # Octant children
        self.is_leaf = True
    ...
    def insert(
        ...
        if self.is_leaf:
            if self.particle_idx == -1:
                ...
                old_pos = pos.copy()  # Will be updated below
                self.particle_idx = -1
                self.is_leaf = False
                ...
                # Need to recalculate old position from COM tracking
                # For simplicity, we'll use a simplified approach

        # Insert into appropriate child
        octant = self.get_octant(pos)
        self.children[octant].insert(pos, mass, idx, max_depth, depth + 1)
...
class NBodyGravity(BasePattern, GPUMixin):
    ...
    def __init__(self, config: Optional[NBodyConfig] = None):
    ...
    def _build_tree(self) -> OctreeNode:
        ...
        for i in range(self.config.n_particles):
            root.insert(self.positions[i], self.masses[i], i)
        ...
        for i in range(n):
            for j in range(i + 1, n):
                ...
                if r > self.config.softening:
                    f = self.config.G * self.masses[i] * self.masses[j] / r**3
                    forces[i] += f * dx
                    forces[j] -= f * dx
    ...
    def _integrate_euler(self, forces: np.ndarray):
        """Euler integration step."""
        dt = self.config.dt
        self.velocities += forces / self.masses[:, np.newaxis] * dt
        self.positions += self.velocities * dt

    def _integrate_leapfrog(self, forces: np.ndarray):
        ...
        # Half step velocity
        self.velocities += forces_new / self.masses[:, np.newaxis] * dt / 2

    def _integrate_rk4(self, forces: np.ndarray):
    ...
    def run(self, hypothesis: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        return {
            ...
            "com": np.average(self.positions, axis=0, weights=self.masses),
