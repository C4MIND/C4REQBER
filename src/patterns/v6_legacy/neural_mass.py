...
class NeuralMassPattern(SimulationPattern):
    ...
    def __init__(self):
        ...
        self.noise_stream: Optional[np.random.Generator] = None
    ...
    async def _jansen_rit_simulation(self) -> Dict[str, Any]:
        ...
        logs = [
            f"Jansen-Rit neural mass simulation completed",
    ...
    async def _wendling_simulation(self) -> Dict[str, Any]:
        ...
        metrics["model"] = "wendling"
        
        logs = [
            f"Wendling neural mass simulation completed",
            f"Slow inhibition included for epileptic dynamics",
            f"Dominant frequency: {metrics['dominant_freq']:.2f} Hz",
        ]
    ...
    async def _wilson_cowan_simulation(self) -> Dict[str, Any]:
        ...
        logs = [
            f"Wilson-Cowan simulation completed",
    ...
    def _sigmoid(self, v: np.ndarray) -> np.ndarray:
        ...
        cfg = self.config
        return 2 * cfg.e0 / (1 + np.exp(cfg.r * (cfg.v0 - v)))
    ...
    def _calculate_eeg_metrics(
        ...
        firing_e: Optional[np.ndarray], firing_p: Optional[np.ndarray],
        firing_i: Optional[np.ndarray]
    ) -> Dict[str, float]:
