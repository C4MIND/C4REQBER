"""
TURBO-CDI v6.0 - Opinion Dynamics Pattern
...
def _build_network(n: int, network_type: str, param: float) -> np.ndarray:
    ...
    if network_type == "complete":
        ...
        adj = np.random.random((n, n)) < param
...
class OpinionDynamicsPattern:
    ...
    def __init__(self, config: Optional[OpinionDynamicsConfig] = None):
        self.config = config or OpinionDynamicsConfig()
        self.opinions: Optional[np.ndarray] = None
    ...
    def _degroot_step(self) -> np.ndarray:
        ...
        return self.influence_matrix @ self.opinions
    ...
    def _hk_step(self) -> np.ndarray:
        ...
        for i in range(cfg.n_agents):
            ...
            distances = np.linalg.norm(self.opinions - self.opinions[i], axis=1)
            neighbors = distances < cfg.confidence_bound

            if np.sum(neighbors) > 0:
                # Average opinions of neighbors
                new_opinions[i] = np.mean(self.opinions[neighbors], axis=0)
            else:
    ...
    def _format_output(
        ...
        final_opinions = self.opinions.flatten()
        ...
        # Network metrics
        network_density = np.sum(self.network) / (cfg.n_agents * (cfg.n_agents - 1))
        ...
        return {
            ...
            "network": {
                ...
                "adjacency": self.network.tolist() if cfg.n_agents <= 20 else None,
            },
            "config": {
...
if __name__ == "__main__":
    ...
    print(f"✓ Stubbornness prevents full consensus")
