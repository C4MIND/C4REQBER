...
class ProspectTheoryModel:
    ...
    def __init__(self, config: ProspectTheoryConfig = None):
    ...
    def value_function(self, x: float) -> float:
        ...
        if x >= 0:
            ...
            return -cfg.lambda_param * ((-x) ** cfg.beta)
    ...
    def cumulative_prospect_value(self, outcomes: List[float], 
        ...
        sorted_pairs = sorted(zip(outcomes, probabilities))
        outcomes = np.array([o - cfg.reference_point for o, _ in sorted_pairs])
        probabilities = np.array([p for _, p in sorted_pairs])
        
        # Separate gains and losses
        gain_mask = outcomes >= 0
        ...
        if len(gain_outcomes) > 0:
            ...
            # CPT value for gains
            for v, w in zip(gain_outcomes, weights):
                cpt_value += self.value_function(v) * w
        ...
        if len(loss_outcomes) > 0:
            ...
            for v, w in zip(loss_outcomes, weights):
                cpt_value += self.value_function(v) * w
    ...
    def expected_utility(self, outcomes: List[float], 
                         ...
                         utility_fn: Callable = None) -> float:
        """
        ...
        if utility_fn is None:
            utility_fn = lambda x: np.log(x + 100) if x + 100 > 0 else -100
        
        return sum(p * utility_fn(o) for o, p in zip(outcomes, probabilities))
    ...
    def analyze_gamble(self, outcomes: List[float], 
        ...
        def certainty_equivalent(cpt_val):
            if cpt_val >= 0:
                return cpt_val ** (1 / self.config.alpha)
            else:
                return -((-cpt_val) / self.config.lambda_param) ** (1 / self.config.beta)
        
        ce = certainty_equivalent(cpt_value)
    ...
    def fourfold_pattern(self) -> Dict[str, Any]:
        ...
        for key, g in gambles.items():
            analysis = self.analyze_gamble(g["outcomes"], g["probs"], key)
        ...
        return {
            ...
            "certainty_equivalents": dict(zip(gambles.keys(), cpt_ces))
        }
    ...
    def loss_aversion_analysis(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "equivalent_losses": dict(zip(gains, loss_equivalents))
        }
...
class TestProspectTheoryModel(unittest.TestCase):
    ...
    def test_cpt_calculation(self):
        ...
        cpt = model.cumulative_prospect_value(outcomes, probs)
