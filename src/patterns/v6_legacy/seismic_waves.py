"""
TURBO-CDI v6.0 - Seismic Waves Pattern
...
class SeismicWavesPattern:
    ...
    def __init__(self, config: Optional[SeismicWavesConfig] = None):
    ...
    def _calculate_cfl(self) -> float:
        ...
        dx = self.x[1] - self.x[0]
        return self.vmax * self.config.dt / dx
    ...
    def run(self, hypothesis: Dict[str, Any] = None) -> Dict[str, Any]:
        """Run the seismic wave simulation"""
        cfg = self.config
...
class TestSeismicWaves(unittest.TestCase):
    ...
    def test_source_time_function(self):
