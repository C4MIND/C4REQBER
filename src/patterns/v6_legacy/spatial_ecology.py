...
class SpatialEcologyConfig:
    ...
    carrying_capacities: Optional[List[float]] = None
    diffusion_coeffs: Optional[List[float]] = None
...
class SpatialEcologyPattern(SimulationPattern):
    ...
    def __init__(self):
        ...
        self.config: Optional[SpatialEcologyConfig] = None
    ...
    async def _simulate(self, hypothesis: Hypothesis) -> Dict[str, Any]:
        ...
        for step in range(self.config.n_steps):
            if self.config.model_type == "fisher_kpp":
                self._step_fisher_kpp(dx, dt)
            elif self.config.model_type == "turing":
                self._step_turing(dx, dt)
            elif self.config.model_type == "competition":
    ...
    def _step_fisher_kpp(self, dx: float, dt: float) -> None:
        ...
        u = self.fields[0]
        D = self.config.diffusion_coeffs[0]
        r = self.config.growth_rates[0]
        K = self.config.carrying_capacities[0]
    ...
    def _step_turing(self, dx: float, dt: float) -> None:
        ...
        Da = self.config.activator_diffusion
        Di = self.config.inhibitor_diffusion
    ...
    def _step_competition(self, dx: float, dt: float) -> None:
        ...
        for i, u in enumerate(self.fields):
            D = self.config.diffusion_coeffs[i]
            r = self.config.growth_rates[i]
            K = self.config.carrying_capacities[i]
    ...
    def _record(self, t: float) -> None:
        ...
        if self.config.model_type in ["fisher_kpp", "invasion"]:
            # Find radius where density > threshold
            N = self.config.grid_size
            center = N // 2
            threshold = 0.01 * self.config.carrying_capacities[0]
            
            mask = u > threshold
            if np.any(mask):
                y, x = np.where(mask)
                distances = np.sqrt((x - center)**2 + (y - center)**2)
                spread_radius = np.max(distances) * self.config.dx
            else:
                spread_radius = 0
        else:
            spread_radius = 0
    ...
    def _analyze_results(self) -> Dict[str, Any]:
        ...
        logs = [
            f"Spatial Ecology ({self.config.model_type}): {self.config.grid_size}×{self.config.grid_size}",
    ...
    def _calculate_confidence(self, results: Dict[str, Any]) -> float:
        ...
        if self.config.model_type == "turing" and "pattern_wavelength" in metrics:
            factors.append(0.3)
