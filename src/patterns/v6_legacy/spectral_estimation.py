"""
TURBO-CDI v6.0 - Spectral Estimation Pattern
...
class SpectralEstimationConfig:
    ...
    # FFT parameters
    nfft: Optional[int] = None  # FFT length (None = nperseg)
...
class SpectralEstimationPattern:
    ...
    def __init__(self, config: Optional[SpectralEstimationConfig] = None):
    ...
    def _generate_test_signal(self) -> np.ndarray:
        ...
        noise_power = 0.1
        return signal + noise_power * noise
    ...
    def _periodogram(self, x: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        # Frequencies
        freqs = np.fft.fftfreq(nfft, 1 / cfg.sampling_rate)
    ...
    def _welch(self, x: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        psd_sum = np.zeros(cfg.nfft // 2 + 1 if cfg.return_onesided else cfg.nfft)
        ...
        # Frequencies
        freqs = np.fft.fftfreq(cfg.nfft, 1 / cfg.sampling_rate)
        if cfg.return_onesided:
            freqs = freqs[: cfg.nfft // 2 + 1]
    ...
    def _format_output(
        ...
        centroid = np.trapz(freqs * psd, freqs) / total_power if total_power > 0 else 0
        bandwidth = (
            np.sqrt(np.trapz((freqs - centroid) ** 2 * psd, freqs) / total_power)
            if total_power > 0
            else 0
        )
        ...
        return {
            ...
            "frequency_resolution": cfg.sampling_rate / cfg.nfft,
            "config": {
                "nperseg": cfg.nperseg,
...
def test_multitaper_resolution():
...
def test_total_power_conservation():
