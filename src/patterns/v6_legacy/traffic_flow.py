"""
TURBO-CDI v6.0 - Traffic Flow Pattern
...
class TrafficFlowConfig:
    ...
    critical_density: Optional[float] = None  # vehicles/km/lane (None = auto)
...
class TrafficFlowPattern:
    ...
    def __init__(self, config: Optional[TrafficFlowConfig] = None):
        self.config = config or TrafficFlowConfig()

        # Fields
        self.density: Optional[np.ndarray] = None  # Vehicles per km
        self.velocity: Optional[np.ndarray] = None  # km/h
        self.flux: Optional[np.ndarray] = None  # Vehicles per hour
        ...
        if cfg.fundamental_diagram == FundamentalDiagram.GREENSHELDS:
            ...
            Q[mask2] = (
                ...
                * (cfg.jam_density - rho[mask2])
                / (cfg.jam_density - cfg.critical_density)
            )
    ...
    def _lwr_step(self):
        ...
        Q, v = self._fundamental_diagram(self.density)
        ...
        for i in range(1, cfg.n_cells):
            # Rusanov flux
            Q_left, _ = self._fundamental_diagram(np.array([self.density[i - 1]]))
            Q_right, _ = self._fundamental_diagram(np.array([self.density[i]]))
        ...
        if cfg.bc_type == BoundaryCondition.INFLOW_OUTFLOW:
            ...
            Q_interface[0] = min(
                cfg.inflow_rate,
                self._fundamental_diagram(np.array([self.density[0]]))[0][0],
            )
            # Outflow (free flow)
            Q_interface[-1] = self._fundamental_diagram(np.array([self.density[-1]]))[
                0
            ][0]
        else:
    ...
    def _update_fields(self):
        ...
        self.flux, self.velocity = self._fundamental_diagram(self.density)
    ...
    def _compute_travel_time(self) -> float:
        ...
        return self.config.road_length / avg_speed * 3600  # seconds
    ...
    def run(self, hypothesis: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        for step in range(n_steps):
            ...
            if step % cfg.output_interval == 0:
                self.density_history.append(self.density.copy())
                self.flux_history.append(self.flux.copy())
    ...
    def _format_output(self) -> Dict[str, Any]:
        ...
        is_congested = max_density > congestion_threshold
        ...
        output = {
            ...
            "final_density": self.density.tolist(),
            "final_velocity": self.velocity.tolist()
            if cfg.model != TrafficModel.CA
            else self.velocity.tolist(),
            "parameters": {
...
def test_fundamental_diagram_monotonicity():
...
def test_bottleneck_creates_congestion():
