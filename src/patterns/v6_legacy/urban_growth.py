"""
TURBO-CDI v6.0 - Urban Growth Pattern
...
class UrbanGrowthConfig:
    ...
    # Initial conditions
    seed_location: Optional[Tuple[int, int]] = None
...
class UrbanGrowthPattern:
    ...
    def __init__(self, config: Optional[UrbanGrowthConfig] = None):
        self.config = config or UrbanGrowthConfig()
        self.grid: Optional[np.ndarray] = None  # 0=empty, 1=urban, 2=road
        self.land_use: Optional[np.ndarray] = None  # Land use types
    ...
    def _create_roads(self):
        ...
        self.road_cells = list(zip(*np.where(self.grid == 2)))

    def _spawn_agents(self):
        """Create initial population"""
        cfg = self.config

        urban_cells = list(zip(*np.where(self.grid == 1)))
    ...
    def _calculate_suitability(self, y: int, x: int) -> float:
        ...
        if self.grid[y, x] == 1:  # Already urban
            return 0.0

        if self.grid[y, x] == 2:  # Road
            return 0.3
    ...
    def _ca_spontaneous_growth(self):
        ...
        for _ in range(n_spontaneous):
            ...
            if self.grid[y, x] == 0:
                ...
                if np.random.random() < suitability * 0.1:
                    self.grid[y, x] = 1
                    self.land_use[y, x] = np.random.choice(
                        ["residential", "commercial"]
                    )

    def _ca_diffusion(self):
                    ...
                    if 0 <= ny < cfg.height and 0 <= nx < cfg.width:
                        if self.grid[ny, nx] == 0:
                            ...
                            if np.random.random() < prob:
                                self.grid[ny, nx] = 1
                                self.land_use[ny, nx] = self.land_use[y, x]

    def _agent_relocation(self):
        ...
        for agent in self.agents:
            ...
            if np.random.random() < 0.05:
                ...
                for _ in range(10):
                    ...
                    if self.grid[cy, cx] == 1:  # Must be urban
    ...
    def _location_score(self, y: int, x: int, agent_type: str) -> float:
        ...
        if agent_type == "resident":
            ...
            if self.land_use[y, x] == "residential":
                score += 1.0
            elif self.land_use[y, x] == "industrial":
                score -= 0.5
            ...
            for dy in [-2, -1, 0, 1, 2]:
                for dx in [-2, -1, 0, 1, 2]:
                    ny, nx = y + dy, x + dx
                    if 0 <= ny < self.config.height and 0 <= nx < self.config.width:
                        if self.land_use[ny, nx] == "commercial":
                            score += 0.1

        elif agent_type == "business":
            # Prefer commercial areas
            if self.land_use[y, x] == "commercial":
                score += 1.0
            # Prefer road access
            for ry, rx in self.road_cells:
                dist = abs(y - ry) + abs(x - rx)
    ...
    def _format_output(self) -> Dict[str, Any]:
        ...
        if len(urban_cells[0]) > 0:
            ...
            y_center = x_center = y_spread = x_spread = 0
        ...
        return {
            ...
            "final_land_use": self.land_use.tolist(),
            "statistics": {
