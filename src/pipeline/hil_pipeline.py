...
class HILDiscoveryPipeline(BasePipeline):
    ...
    async def _regenerate_hypotheses(self, topic: str, gaps: list[dict[str, Any]], sources: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    async def discover(self, topic: str, competing_hypotheses: int = 2, no_iterative: bool = False) -> DiscoveryRecord:
        ...
        try:
            ...
            class _DiscoveryHandler(CommandHandler):
                def handle(self, cmd):
                    logger.debug("CQRS: StartDiscoveryCommand handled for %s", cmd.problem_statement)
            bus = CqrsBus()
            bus.register_command(StartDiscoveryCommand, _DiscoveryHandler())
            cmd = StartDiscoveryCommand(problem_statement=topic, domain="general", complexity_level=1)
            bus.execute(cmd)
        ...
        await event_bus.emit("pipeline_start", {
            "topic": topic, "query_type": query_type, "config": cfg.to_dict()
        }, mode="turbo")

        # Phase A: USP Cognitive Framing
        await event_bus.emit("phase_start", {"phase": "A", "name": "USP Cognitive Framing"}, mode="turbo")
        ...
        await event_bus.emit("phase_end", {"phase": "A", "name": "USP Cognitive Framing"}, mode="turbo")

        # Phase B: Knowledge Acquisition
        await event_bus.emit("phase_start", {"phase": "B", "name": "Knowledge Acquisition"}, mode="turbo")
        phase_b = PhaseB_KnowledgeAcquisition(api_keys=cfg.api_keys)
        bibliography = await phase_b.run(topic, min_sources=cfg.min_sources, fallback_to_web=cfg.fallback_to_web_search)
        record.sources = bibliography
        record.bibliography = bibliography[:cfg.max_sources]
        await event_bus.emit("phase_end", {"phase": "B", "name": "Knowledge Acquisition"}, mode="turbo")
        ...
        await event_bus.emit("phase_start", {"phase": "D", "name": "Cognitive Agents"}, mode="turbo")
        ...
        await event_bus.emit("phase_start", {"phase": "E", "name": "Simulation & Verification"}, mode="turbo")
        ...
        if record.hypotheses:
            ...
            if sim_result.get("status") == "failed" and any(keyword in str(sim_result.get("raw_output", "")).lower() for keyword in ("insufficient", "resource", "out of memory", "cuda", "exceeded", "unavailable")):
        ...
        await event_bus.emit("phase_end", {"phase": "E", "name": "Simulation & Verification"}, mode="turbo")

        # Phase F: Dissertation Generation
        await event_bus.emit("phase_start", {"phase": "F", "name": "Dissertation Generation"}, mode="turbo")
        phase_f = PhaseF_DissertationGeneration()
        diss, plugins_context = phase_f.run(topic=topic, record=record, c4_state=usp_context.get("c4_state", "unknown"))
        record.plugins_context.update(plugins_context)
        await event_bus.emit("phase_end", {"phase": "F", "name": "Dissertation Generation"}, mode="turbo")
        ...
        record.quality_report = self.quality.evaluate(
            ...
            record.simulation, record.verification, record.bibliography, diss
        )
        ...
        if record.quality_report.overall_score < cfg.min_quality_score:
            ...
            if observer.should_halt():
                ...
                record.quality_report.recommendations.append(
                    "Observer: should_halt triggered"
                )
        ...
        if observer.should_halt():
            ...
            record.quality_report.recommendations.append("Stagnation: refine hypothesis or broaden search")

        path = DissertationGenerator().save(diss, filename=f"HIL_v2_{topic.replace(' ', '_')[:30]}.md")
        ...
        try:
            verif_report = await verifier.verify(
                result={"hypothesis": {"text": record.hypotheses[0].get("text", "")} if record.hypotheses else {},
                        "abort_reasons": getattr(record, "abort_reasons", []),
            ...
            if not verif_report.passed:
                print(f"⚠ FinalVerifier: {verif_report.issue_count} issues — max_similarity={verif_report.max_similarity:.2f} to '{verif_report.closest_paper[:60]}'")
                record.quality_report.recommendations.append(f"FinalVerifier: {verif_report.issue_count} issues (max_similarity={verif_report.max_similarity:.2f})")
        except Exception:
            logger.exception("FinalVerifier crash — post-pipeline verification skipped")
            record.quality_report.recommendations.append("FinalVerifier: CRASH (verification unavailable)")

        assert record.quality_report is not None
        await self.emit_event("pipeline_complete", {
            "topic": topic, "path": path, "grade": record.quality_report.grade,
            "score": record.quality_report.overall_score, "sources": len(record.sources),
    ...
    def _classify_query(self, topic: str) -> str:
        ...
        scientific = ["hypothesis", "theory", "mechanism", "quantum", "neuro", "epigenetic", "research", "study", "experiment"]
        practical = ["career", "business", "startup", "how to", "tutorial", "guide", "money", "build"]
...
def run_hil_pipeline(topic: str, config: PipelineConfig | None = None, user_profile: UserProfile | None = None) -> None:
    """CLI entry point."""
