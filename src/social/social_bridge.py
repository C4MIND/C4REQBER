...
async def transfer_to_drafts(
    discovery_record: Any, social_config: dict[str, Any] | None = None
) -> dict[str, Any]:
    """Hook called by pipeline after Phase G. Copies dissertation to drafts."""
    DRAFTS_DIR.mkdir(parents=True, exist_ok=True)

    topic = (
        getattr(discovery_record, "topic", "discovery")
        if hasattr(discovery_record, "topic")
        else "discovery"
    )
    ...
    # Draft state
    (draft_dir / "draft_state.json").write_text(
        json.dumps(
            {
                "id": draft_id,
