"""
TRIZ 76 Standard Solutions Database.

The 76 Standard Solutions are organized into 5 classes:
  Class 1: 13 solutions — Establish or modify Su-Fields
  Class 2: 23 solutions — Remove harmful effects
  Class 3:  6 solutions — System transition
  Class 4: 17 solutions — Detection and measurement
  Class 5: 17 solutions — Simplification and strategy

Each solution includes:
  - id: Standard notation (e.g., "1.1.1")
  - name: Short title
  - description: Full explanation
  - applicability: Predicate function (SuFieldModel -> bool)
  - c4_trajectory: List of C4 state tuples
...
ALL_STANDARD_SOLUTIONS: list[StandardSolution] = (
    CLASS_1_SOLUTIONS
    + CLASS_2_SOLUTIONS
    + CLASS_3_SOLUTIONS
    + CLASS_4_SOLUTIONS
...
SOLUTIONS_BY_ID: dict[str, StandardSolution] = {s.id: s for s in ALL_STANDARD_SOLUTIONS}

SOLUTIONS_BY_CLASS: dict[str, list[StandardSolution]] = {
    "Class 1": CLASS_1_SOLUTIONS,
    "Class 2": CLASS_2_SOLUTIONS,
    "Class 3": CLASS_3_SOLUTIONS,
    "Class 4": CLASS_4_SOLUTIONS,
    "Class 5": CLASS_5_SOLUTIONS,
}


def get_solution(solution_id: str) -> StandardSolution | None:
    """Get a standard solution by its ID (e.g., '1.1.1')."""
    return SOLUTIONS_BY_ID.get(solution_id)


def get_solutions_by_class(class_name: str) -> list[StandardSolution]:
    """Get all solutions in a class (e.g., 'Class 1')."""
    return SOLUTIONS_BY_CLASS.get(class_name, [])


def search_solutions(query: str) -> list[StandardSolution]:
    """Search solutions by name or description (case-insensitive)."""
    query_lower = query.lower()
    results = []
    for sol in ALL_STANDARD_SOLUTIONS:
        # For IDs, prefer prefix match to avoid "2.1" matching "1.2.1"
        id_match = sol.id.startswith(query_lower)
        name_match = query_lower in sol.name.lower()
        desc_match = query_lower in sol.description.lower()
        if id_match or name_match or desc_match:
            results.append(sol)
    return results


def get_all_solutions() -> list[StandardSolution]:
    """Return all 76 standard solutions."""
    return ALL_STANDARD_SOLUTIONS


def count_solutions() -> dict[str, int]:
    """Return counts by class and total."""
    return {
        "Class 1": len(CLASS_1_SOLUTIONS),
        "Class 2": len(CLASS_2_SOLUTIONS),
        "Class 3": len(CLASS_3_SOLUTIONS),
        "Class 4": len(CLASS_4_SOLUTIONS),
        "Class 5": len(CLASS_5_SOLUTIONS),
        "total": len(ALL_STANDARD_SOLUTIONS),
    }
