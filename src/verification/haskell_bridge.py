...
def _validate_haskell_module_name(module_name: str) -> str:
    import re

    if not re.fullmatch(r"[A-Za-z_][A-Za-z0-9_]*", module_name or ""):
        raise ValueError(f"Invalid Haskell module name: {module_name!r}")
    return module_name
...
def verify_haskell_typecheck(code: str, module_name: str = "Verified") -> dict[str, Any]:
    ...
    try:
        module_name = _validate_haskell_module_name(module_name)
    except ValueError as exc:
        return {"status": "error", "error": str(exc), "backend": "haskell-typecheck"}
    ghc = _find_ghc()
    if not ghc:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        try:
            ...
            return {
                "status": "timeout",
                "backend": "haskell-typecheck",
                "error": "GHC type check timed out (30s)",
            }
        except Exception as e:
            return {"status": "error", "backend": "haskell-typecheck", "error": str(e)}


def verify_haskell_quickcheck(
    code: str, module_name: str = "Properties", prop_count: int = 100
) -> dict[str, Any]:
    """Run QuickCheck on property definitions found in the code."""
    try:
        module_name = _validate_haskell_module_name(module_name)
    except ValueError as exc:
        return {"status": "error", "error": str(exc), "backend": "haskell-quickcheck"}
    ghc = _find_ghc()
    if not ghc:
    ...
    if "quickCheck" not in code and "prop_" not in code:
        return {
            ...
            "message": "No QuickCheck properties found in code",
        }
        ...
        try:
            ...
            if compile_result.returncode != 0:
                return {
                    ...
                    "backend": "haskell-quickcheck",
                    "error": compile_result.stderr[-300:],
                }
