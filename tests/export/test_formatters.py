"""
Tests for src/patterns/format/core.py and src/patterns/format/utils.py

Covers:
- PatternResultFormatter.format() dispatch (markdown, text, html, json)
...
- Table generation (markdown tables, HTML tables)
- Code block formatting (error blocks, pre tags)
...
# ═══════════════════════════════════════════════════════════════════
# PatternResultFormatter.format() — Markdown
# ═══════════════════════════════════════════════════════════════════
...
class TestFormatMarkdown:
    ...
    def test_markdown_table_generation(self, formatter, monte_carlo_result):
        md = formatter.format(monte_carlo_result, "markdown")
        lines = md.split("\n")
        table_lines = [l for l in lines if l.startswith("|")]
        assert len(table_lines) >= 2  # header + separator
        assert "| Metric | Value |" in table_lines[0]
        assert "|--------|-------|" in table_lines[1]


# ═══════════════════════════════════════════════════════════════════
# PatternResultFormatter.format() — Text
# ═══════════════════════════════════════════════════════════════════
...
class TestFormatText:
    ...
    def test_text_no_logs(self, formatter, empty_result):
        text = formatter.format(empty_result, "text")
        assert "Logs:" not in text


# ═══════════════════════════════════════════════════════════════════
# PatternResultFormatter.format() — HTML
# ═══════════════════════════════════════════════════════════════════
...
class TestFormatHtml:
    ...
    def test_html_empty_result(self, formatter, empty_result):
        html = formatter.format(empty_result, "html")
        assert "</div>" in html


# ═══════════════════════════════════════════════════════════════════
# PatternResultFormatter.format() — JSON
# ═══════════════════════════════════════════════════════════════════
...
class TestFormatJson:
    ...
    def test_json_pretty_print(self, formatter, agent_based_result):
        js = formatter.format(agent_based_result, "json")
        assert "\n" in js
        assert "  " in js
...
class TestFormatUnknown:
    ...
    def test_unknown_format_lists_supported(self, formatter, agent_based_result):
        with pytest.raises(ValueError) as exc_info:
            formatter.format(agent_based_result, "yaml")
...
class TestFormatForSynthesis:
    ...
    def test_synthesis_no_logs(self, formatter):
        result = {
            "pattern_id": "test",
            "status": "completed",
    ...
    def test_synthesis_key_metrics_limited(self, formatter, agent_based_result):
        synth = formatter.format_for_synthesis(agent_based_result)
        # Should not include all metrics, just key ones
        assert "Key metrics:" in synth
...
class TestFormatForDisplay:
    ...
    def test_display_no_result_key(self, formatter):
        result = {
            "pattern_id": "test",
            "status": "completed",
        }
...
class TestDetectResultType:
    ...
    def test_detect_all_pattern_types(self, pattern_type_map):
        for pattern_id, expected_type in pattern_type_map.items():
            result = {"pattern_id": pattern_id, "result": {}}
            assert _detect_result_type(result, pattern_type_map) == expected_type
...
class TestMdSystemDynamics:
    ...
    def test_stability_stable(self):
        lines = _md_system_dynamics({"metrics": {"is_stable": True}})
        assert "**System stability:** stable" in lines

    def test_stability_unstable(self):
        lines = _md_system_dynamics({"metrics": {"is_stable": False}})
        assert "**System stability:** unstable" in lines

    def test_chaotic(self):
        lines = _md_system_dynamics({"metrics": {"is_chaotic": True, "chaos_indicator_k": 0.8}})
...
# ═══════════════════════════════════════════════════════════════════
# Formatter initialization and edge cases
# ═══════════════════════════════════════════════════════════════════
...
class TestEdgeCases:
    ...
    def test_format_none_values_in_metrics(self, formatter):
        """None values in metrics may crash formatting; test current behavior."""
        result = {
            "pattern_id": "test",
            "status": "completed",
    ...
    def test_format_all_pattern_types(self, formatter):
        for pattern_id in formatter.PATTERN_TYPE_MAP:
            result = {
                "pattern_id": pattern_id,
                "status": "completed",
                "result": {"metrics": {"foo": 1}, "logs": []},
            }
            for fmt in ["markdown", "text", "html", "json"]:
                output = formatter.format(result, fmt)
