...
class TestPatternRegistry:
    ...
    def test_patterns_registered(self):
        """Test that all patterns are auto-registered"""
        ...
        for category in library.values():
            all_patterns.extend([p["id"] for p in category])
        
        assert "monte_carlo" in all_patterns
        assert "agent_based" in all_patterns
        assert "system_dynamics" in all_patterns
        assert "circuit_simulation" in all_patterns
...
class TestIntegration:
    """Test integration between patterns and engine"""
    ...
    async def test_full_pipeline(self, engine):
        """Test complete pipeline with agent-based simulation"""
        hypothesis = Hypothesis(
            title="Market adoption dynamics",
            description="Agent-based simulation of product diffusion",
            parameters={
                "n_agents": 50,
                "n_steps": 100,
                "network_type": "small_world",
        ...
        # Run full pipeline (without evolution for speed)
        ...
        assert "initial_simulation" in results
        assert "validation_report" in results
        assert results["initial_simulation"] is not None
