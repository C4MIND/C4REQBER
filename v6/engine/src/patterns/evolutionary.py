...
class EvolutionaryPattern(SimulationPattern):
    ...
    def __init__(self):
        ...
        self.config: Optional[EvolutionaryConfig] = None
    ...
    async def _simulate(self, hypothesis: Hypothesis) -> Dict[str, Any]:
        ...
        for realization in range(self.config.n_realizations):
            if self.config.model_type == "moran":
                trajectory, fix_data = self._run_moran()
            elif self.config.model_type == "wright_fisher":
    ...
    def _run_wright_fisher(self) -> tuple:
    ...
    def _run_replicator(self) -> tuple:
        """Run replicator dynamics"""
        n_gen = self.config.n_generations
        s = self.config.selection_strength
        
        x = self.config.initial_frequency  # Frequency of type A
        trajectory = [x]
    ...
    def _analyze_results(self) -> Dict[str, Any]:
        ...
        logs = [
            f"Evolutionary Dynamics ({self.config.model_type}): N={N}, s={s}",
