...
class ForestGapPattern(SimulationPattern):
    ...
    def __init__(self):
        ...
        self.species_grid: Optional[np.ndarray] = None
        self.age_grid: Optional[np.ndarray] = None
    ...
    def _initialize_grid(self):
        ...
        self.grid[forest_mask] = self.rng.integers(1, 4, np.sum(forest_mask))
        self.species_grid[forest_mask] = self.rng.integers(0, cfg.num_species, np.sum(forest_mask))
        self.age_grid[forest_mask] = self.rng.integers(0, 100, np.sum(forest_mask))
    ...
    async def _simulate(self) -> Dict[str, Any]:
        ...
        for year in range(cfg.years):
            ...
            if year % cfg.record_interval == 0:
                ...
                # Calculate metrics
                cover = np.sum(self.grid > 0) / (N * N)
                cover_history.append(float(cover))
                
                # Species richness
                present_species = np.unique(self.species_grid[self.grid > 0])
                ...
                # Age distribution
                ages = self.age_grid[self.grid > 0]
        ...
        logs = [
            f"Forest gap simulation completed",
        ...
        return {
            ...
            "final_grid": self.grid.tolist(),
            "final_species": self.species_grid.tolist(),
    ...
    def _apply_disturbance(self):
        ...
        # Apply disturbance
        self.grid[disturbance_mask] = 0  # Empty
        self.species_grid[disturbance_mask] = -1  # No species
        self.age_grid[disturbance_mask] = 0
        
        # Gap expansion (neighbor effect)
        for _ in range(3):  # Multiple expansion attempts
            for i in range(N):
                for j in range(N):
                    if self.grid[i, j] == 0:  # If gap exists
                        ...
                        for ni, nj in neighbors:
                            if self.grid[ni, nj] > 0 and self.rng.random() < cfg.gap_expansion_prob:
                                self.grid[ni, nj] = 0
                                self.species_grid[ni, nj] = -1
                                self.age_grid[ni, nj] = 0
    
    def _apply_succession(self):
        ...
        for i, j in cells:
            ...
            # State transitions
            if self.grid[i, j] == 0:  # Empty -> Juvenile (recruitment)
                ...
                if self.rng.random() < rate:
                    self.grid[i, j] = 1  # Juvenile
                    self.species_grid[i, j] = self._choose_species(i, j)
                    self.age_grid[i, j] = 0
            
            elif self.grid[i, j] == 1:  # Juvenile
                # Mortality
                ...
                if self.rng.random() < cfg.maturation_rate:
                    self.grid[i, j] = 2  # Mature
            
            elif self.grid[i, j] == 2:  # Mature
                # Mortality
                ...
                if self.rng.random() < cfg.aging_rate:
                    self.grid[i, j] = 3  # Old growth
            
            elif self.grid[i, j] == 3:  # Old growth
                # Higher mortality
    ...
    def _has_mature_neighbor(self, i: int, j: int) -> bool:
        ...
        for ni, nj in neighbors:
            if self.grid[ni, nj] >= 2:  # Mature or old growth
                return True
        return False
    ...
    def _choose_species(self, i: int, j: int) -> int:
        ...
        neighbor_species = []
        for ni, nj in neighbors:
            if self.species_grid[ni, nj] >= 0:
                neighbor_species.append(self.species_grid[ni, nj])
        
        if neighbor_species and self.rng.random() < 0.8:
            # 80% chance to recruit from neighbor species
            return self.rng.choice(neighbor_species)
        else:
    ...
    def _calculate_metrics(
        ...
        # Stage distribution
        stage_counts = np.bincount(self.grid.flatten(), minlength=4)
        ...
        if final_richness > 0:
            species_counts = np.bincount(
                self.species_grid[self.grid > 0].flatten(),
