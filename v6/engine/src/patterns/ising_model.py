...
class IsingModelPattern(SimulationPattern):
    ...
    def __init__(self):
        ...
        self.config: Optional[IsingConfig] = None
    ...
    async def _simulate(self, hypothesis: Hypothesis) -> Dict[str, Any]:
        ...
        elif self.config.algorithm == "wolff":
            await self._wolff()
        elif self.config.algorithm == "swendsen_wang":
    ...
    async def _metropolis(self) -> None:
        ...
        for sweep in range(self.config.n_sweeps + self.config.thermalization):
            ...
            if sweep >= self.config.thermalization:
                if (sweep - self.config.thermalization) % self.config.measure_every == 0:
                    self._measure()
    ...
    def _measure(self) -> None:
        ...
        for i in range(L):
            for j in range(L):
                ...
                E -= self.config.J * s * neighbors
        E = E / N - self.config.h * np.sum(self.lattice) / N
        self.measurements["energy"].append(E)
        self.measurements["energy_squared"].append(E * E)

    def _analyze_results(self) -> Dict[str, Any]:
        """Analyze simulation results"""
        if not self.measurements["magnetization"]:
        ...
        logs = [
            f"Ising model: {L}×{L} lattice, T={T:.3f}, algorithm={self.config.algorithm}",
